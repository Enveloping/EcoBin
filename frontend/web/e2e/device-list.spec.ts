...
function fixtures() {
  const primary = permanentDeviceAsset({
    ...
    listStatus: { faults: ['控制板通信未连接'], observedAt: timestamp, ports: [
      { portNo: 1, displayName: '纸类', reportedWeightGrams: 12345, weightValueAvailable: true,
        ...
        overallFull: true, weightFull: false, fullnessObservedAt: timestamp, infraredValue: 'BLOCKED', infraredSensorHealth: 'OK', faults: [] },
      { portNo: 2, displayName: '瓶类', reportedWeightGrams: 0, weightValueAvailable: false,
        ...
        overallFull: true, weightFull: true, fullnessObservedAt: timestamp, infraredValue: 'CLEAR', infraredSensorHealth: 'SENSOR_FAULT', faults: ['称重传感器故障'] },
      { portNo: 3, displayName: '金属', reportedWeightGrams: 0, weightValueAvailable: true,
        weightSensorHealth: 'OK', weightMeasurementStatus: 'STABLE', observedAt: timestamp,
        overallFull: false, weightFull: null, fullnessObservedAt: null, infraredValue: 'CLEAR', infraredSensorHealth: 'OK', faults: [] },
    ] },
  });
  return [primary, permanentDeviceAsset({ assetUid: '30000000-0000-4000-8000-000000000002', hardwareSn: 'NO-REPORT-DEVICE', expectedPortCount: 2 })];
}
...
test('device list shows fixed-height records, simple condition and aligned port observations', async ({ page, context }) => {
  const requests = await setup(page);
  const headers = page.getByRole('columnheader');
  for (const title of ['状态', '安装地址', '投口重量', '重量满溢', '红外满溢', '归属']) {
  ...
  await expect(row).toContainText('异常');
  await expect(row).toContainText('东门服务站旁的可回收物集中投放点');
  await expect(row).not.toContainText('控制板通信未连接');
  await expect(row).not.toContainText('2 口：称重传感器故障');
  ...
  await expect(unknown).toContainText('正常');
  await expect(unknown).toContainText('尚未设置');
  ...
  const rowHeights = await Promise.all([row, unknown].map(locator => locator.evaluate(element => Math.round(element.getBoundingClientRect().height))));
