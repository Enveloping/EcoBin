...
export default function OrganizationMiniappConfiguration({
  ...
  const saveConfiguration = async () => {
    ...
    if (!configuration && !draft.appSecret.trim()) {
      message.warning('首次配置必须填写 AppSecret');
      return;
    }
  ...
  return (
    <Space direction="vertical" size={16} style={{ width: '100%' }}>
      <Alert
        type={configuration ? 'info' : 'warning'}
        showIcon
        message={
          configuration
      ...
      {configuration && (
        <Form.Item
          label="当前 AppSecret"
          extra={`服务端脱敏值：${configuration.maskedAppSecret}`}
        >
          <Input.Password
            readOnly
      ...
      <Form.Item
        label={configuration ? '轮换 AppSecret' : 'AppSecret'}
      ...
      >
        <Input.Password
          ...
          maxLength={256}
          placeholder={configuration ? '如需轮换，请输入新的 AppSecret' : '请输入 AppSecret'}
          onChange={(event) =>
            setDraft((current) => ({
              ...current,
