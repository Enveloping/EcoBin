...
function decodeDeploymentCode(value?: string): string {
  if (!value) return ''
  try {
    return parseCleaningDeploymentCode(decodeURIComponent(value))
  } catch {
    return ''
  }
}
...
Page({
  ...
  onLoad(options: Record<string, string | undefined>) {
    ...
    const deploymentCode = decodeDeploymentCode(options.deploymentCode)
    const previewOnly = getEntryMode() !== 'CLEANING'
    this.setData({ deploymentCode, previewOnly })
    if (previewOnly) {
      this.setData({
        loading: false,
        stage: 'preview',
    ...
    if (!deploymentCode) {
      this.setData({
        loading: false,
        stage: 'error',
        errorMessage: '设备部署码无效，请返回后重新扫描设备二维码。',
      })
      return
    }

    const restored = restoreCleanOperationIntent()
    if (restored && restored.deploymentCode !== deploymentCode) {
      ...
      this.setData({
        ...
        errorMessage:
          `设备 ${restored.deploymentCode} 还有一笔结果未确定的清运操作，`
          + '为避免重复开门，当前不能发起另一笔操作。',
      })
      return
    }
  ...
  async loadOptions() {
    if (!this.data.deploymentCode || this.data.previewOnly) return
    this.setData({ loading: true, stage: 'loading', errorMessage: '' })
    try {
      const options = await cleanOptions(this.data.deploymentCode, false)
  ...
  async onConfirmStart() {
    ...
    const { deploymentCode, selectedPortNo, installedBagQr } = this.data
