...
function factStatus(status: string | null | undefined): string {
  ...
  return status ? values[status] ?? status : '未知'
}
    ...
    try {
      ...
      this.setData({
        ...
        photos: detail.photos.map((item) => ({
          ...item,
          positionText: PHOTO_POSITION_TEXT[item.position] || item.position,
          statusText: PHOTO_STATUS_TEXT[item.status] || item.status,
        })),
      })
    } catch (error) {
      if (generation !== this.requestGeneration) return
