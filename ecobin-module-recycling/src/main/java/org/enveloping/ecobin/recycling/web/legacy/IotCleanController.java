package org.enveloping.ecobin.recycling.web.legacy;
...
public class IotCleanController {
    ...
    public Result<Void> gross(@Valid @RequestBody CleanGrossRequest request) {
        cleanOrderService.reportGross(new LegacyCleanGrossCommand(
                request.getSn(),
                request.getCleanOrderId(),
                request.getWeight(),
                request.getPhotoOpenOutside(),
                request.getPhotoOpenInside(),
                request.getPhotoCloseOutside(),
                request.getPhotoCloseInside()));
        return Result.ok();
    }
    ...
    public Result<Void> tare(@Valid @RequestBody CleanTareRequest request) {
        cleanOrderService.reportTare(new LegacyCleanTareCommand(
                request.getSn(),
                request.getCleanOrderId(),
                request.getWeight()));
        return Result.ok();
