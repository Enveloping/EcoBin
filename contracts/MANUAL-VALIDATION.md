...
| 同步服务 | 17 |
| 事件 | 21 |
| 总功能点 | 38（低于 OneNet 的 100 个功能点上限） |
| 导入文件 | `173777` bytes、小于 256 KiB、LF 换行 |
| 枚举显示说明 | 1～20 个中英文、数字、下划线或连字符 |
| 单服务输入/输出 | 各不超过 20 |
| 单事件输出 | 不超过 50 |

2026-09-06 当前候选的 SHA-256 为
`39e793b1a72f64ed70a3e8a202d4cf8e38369d3cd98b5c3f944cac45fab030a5`，距离
`262144` bytes 上限还剩 `88367` bytes。导入时必须选择仓库内的生成文件，不要用编辑器
重排 JSON、转换换行或另存副本。
...
`examples/onenet-wire/` 中为全部 17 个服务提供了同格式样例。样例中的 COS 凭证是假的，
只用于类型/协议验证，不能用于真实上传。
...
`examples/onenet-wire/` 中为全部 21 个事件/回执提供了可发布样例。
