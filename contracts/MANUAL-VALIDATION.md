...
| 同步服务 | 15 |
| 事件 | 18 |
| 总功能点 | 33（低于 OneNet 的 100 个功能点上限） |
| 导入文件 | `260285` bytes、小于 256 KiB、LF 换行 |
| 枚举显示说明 | 1～20 个中英文、数字、下划线或连字符 |
| 单服务输入/输出 | 各不超过 20 |
| 单事件输出 | 不超过 50 |

2026-08-29 当前候选的 SHA-256 为
`80e98c318beea54a61488bd96b0fc201af3c80c6ee7323510d7a0071be479c75`，距离
`262144` bytes 上限只剩 `1859` bytes。导入时必须选择仓库内的生成文件，不要用编辑器
重排 JSON、转换换行或另存副本。
...
`examples/onenet-wire/` 中为全部 15 个服务提供了同格式样例。样例中的 COS 凭证是假的，
只用于类型/协议验证，不能用于真实上传。
...
`examples/onenet-wire/` 中为全部 18 个事件/回执提供了可发布样例。
