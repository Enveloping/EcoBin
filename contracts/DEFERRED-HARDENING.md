...
1. **三端完整 payload 校验器**
   让 generated Python、Java 和 MCU C 对 37 种消息执行与 Registry 相同的精确长度、
...
3. **黄金向量覆盖扩充**
   从当前代表性向量扩充为 37/37 消息正例及字段级负例；负例应重算 CRC，避免只在帧层
...
4. **超长粘包的增量排空**
   ...
   `>512 bytes` 全合法粘包轨迹。
5. **共同版本形成后的门禁**
   在真实 Python 3.11、Java 21、MCU 目标工具链和 OneNet 测试产品上固化 CI/HIL。
   这对应 I-055 的后续升级，不替代本轮人工 OneNet 导入和 MCU checkpoint。
...
- F-11 运行时接入时，必须以权威 Schema/Registry 和参考校验器补齐上述能力后再切换正式
  链路。
- H-03 必须在目标 MCU 工具链和真机上验证；本地 Java/Python 通过不代表门锁、断电恢复或
  非易失能力已经验收。
