...
| `/devices` | Web 账号 | `device.read` | `device.configuration.manage`、`device.allocation.manage`、`device.business.manage`；平台固定用例另管理资产、租户分配与验收 | 已接入；平台资产、租户设备池、机构部署、运行事实、配置应用和验收工作台 |
...
| 设备 | 平台资产登记、机构部署、整机/投口运行事实、完整配置版本、应用跟踪、验收和经营开关 | 可用 | OneNet 设备预建、Device Key 写入香橙派和真机验收仍是受控线下步骤 |
...
设备页面保留唯一 `/devices` 入口。平台管理员使用“平台资产 / 租户分配 /
机构部署”三个工作区；租户主体以及同时拥有 `device.read` 和租户级
`device.allocation.manage` 的普通工作人员使用“机构部署 / 租户设备池”；其他
机构工作人员只看得到已授权机构的部署。平台登记资产只建立 EcoBin 本地事实，
不调用 OneNet、不生成或显示 Device Key；平台只能把库存资产分配到租户，不代替
租户选择机构。

同租户调拨是两个明确命令：租户先关闭经营，按部署版本和当前分配版本把设备
退回租户池；之后再选择目标机构创建全新部署。两步之间“租户池中、尚未部署”
是合法可恢复状态；原部署保留为已结束历史，既有订单、清运和统计仍归原机构。

接入抽屉按“接入概况 / 运行事实 / 设备配置 / 验收记录”分层。首次部署由平台
读取就绪证据，阻断解除后仍需分别确认投递门、摄像头和清运门的现场安装；
同租户已验收设备重新部署时按新配置与运行证明自动技术就绪，不重复写入平台
验收记录。平台不显示经营开关；经营仅由租户主体或 `device.business.manage`
能力发起。租户设备池、退池和重新部署另行要求租户级
`device.allocation.manage`，该能力不隐含 `device.read`。配置发布与重同步真实校验
HTTP `202 + Location`，按服务端 `recommendedPollAfterMs` 轮询。当前 `fixed-frame` 运行例外下，
`APPLIED` 只证明香橙派已可靠保存并启用配置，不宣称 MCU 已真实接收。
