...
|---|---|---|---|---|
| `/tenant` | 平台管理员 | `tenant.read` | `tenant.manage` | 已接入；含租户主体密码重置 |
| `/my-tenant` | 租户主体、工作人员 | `tenant.read` | `tenant.manage` | 已接入 |
| `/organizations` | Web 账号 | `organization.read` | `organization.manage` | 已接入 |
| `/organization-users` | Web 账号 | `user.read` | `user.freeze`、`cleaner.manage` | 已接入；含详情、冻结/恢复、清运授权 |
| `/staff` | Web 账号 | `staff.read` | `staff.manage`、`permission.manage` | 已接入 |
| `/user-bindings` | Web 账号 | `user.read` 且 `staff.bind` | `staff.bind` | 已接入；手机号只进入请求体 |
| `/access` | Web 账号 | `permission.read` | `permission.manage` | 已接入 |
...
| 设备 | 资产、部署、投口、配置、容量与健康 | 等待契约 | 对应 paths/schema/examples 与后端纵向能力 |
| 投递 | 订单、审核纠错、待返现与用户钱包查询 | 等待契约 | 原始事实/认定版本/金额字段和状态机契约 |
| 清运 | 清运操作、袋、满溢、基准和恢复 | 等待契约 | 可恢复操作、异常分支和设备结果契约 |
| 资金 | 机构充值、额度、提现审核与渠道状态 | 等待契约 | 金额字符串、双侧冻结、渠道终态和对账契约 |
| 运营 | 概览、告警、审计、对账与技术任务 | 等待契约 | 只读投影、游标分页、任务状态和审计契约 |

等待项不建立模拟业务终态页面，也不复活旧接口。历史页面源码已经删除；对应纵向契约
可运行后，以新页面重新接入。
