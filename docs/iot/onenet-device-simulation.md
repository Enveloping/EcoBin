...
### A.2 导入物模型
进入产品「功能定义 / 设置物模型」，导入 `docs/iot/onenet-thing-model.json`。
> 只为打通链路的话，至少要有 `cleanGross` 事件功能点（输出 `cleanOrderId` int64 + `weight` float）。
> 物模型字段口径见 `docs/iot/onenet-thing-model.md` §4。
...
```
> 后端收到此事件才建单：需先用小程序对该设备「开启设备」建立活跃会话，否则建无主单不返现（见 `docs/iot/onenet-thing-model.md` §8）。
...
> 下行（开门命令）不在本文档范围：需 OneNet 平台 API 凭证（`product-id/access-key`），未到位，见 `docs/planning/open-items.md`。
