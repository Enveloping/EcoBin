...
图片二进制由**设备直传腾讯云 COS**（设备↔COS 公网直连，不经后端、不占后端带宽）。**投递、清运一致**：照片对象 key 由**设备自定位置**，后端不下发 key、不预存 URL，设备直传后把 **4 个 URL 随上行业务事件回传**，后端原样存。两端仅回传事件不同（投递 `deliveryComplete`、清运 `cleanGross`）：
...
> - 照片 key：投递、清运均由**设备自定位置**，URL 随上行事件回传（投递 `deliveryComplete`、清运 `cleanGross`），后端原样存、不再算 key。
...
2026-08-29 已通过生成一致性、平台边界、三语言向量和全部线级样例的本地验证；项目
负责人同时确认该版本此前已经导入目标 OneNet 产品，先前的“待导入”只是文档落后，
无需重复导入。本文件前述 3 服务表格保留早期联调历史，不应再用于缩减当前控制台物模型。
...
- 后端公开 `oneNetConnectionStatus`（传输在线）和 `edgeConnectionStatus`（可信运行快照
  已经建立）两层状态。香橙派在启动、重连和关键运行状态变化后上报快照，并每 5 分钟
  兜底上报一次；后端不会因为快照变旧而自行判定设备离线。
- `10410` 是产品下找不到 `device_name`，阻断设备身份；`10421` 是设备离线，暂停任务且
  不消耗连续失败次数。后端等待 OneNet 上线通知唤醒，不用服务调用或普通上行消息推测
  设备是否在线。
...
> **照片随本次称重一并回传**：投递「上传后建单 + 设备本地继续投递」，开门时后端尚无订单标识、继续投递的本地再开门后端也够不着，故**照片位置由设备决定并随上报回传 4 个 URL，后端原样存**（不再按公式复原）。缺失的槽位前端显示占位图。清运同此（照片位置设备自定、URL 随 `cleanGross` 回传，见 §3.4 / §4.2），仅回传事件不同。
...
| `cleanGross`(含 4 照片 URL) | event | `CleanOrderService.reportGross`（按 `cleanOrderId` 回填毛重 + 原样存设备回传 4 URL，幂等） | 已实现（直连 + OneNet MQ 上行），待联调 |
...
6. ~~**当前物模型生效**~~ ✅ 15 服务 / 18 事件候选的本地 schema 与平台边界校验已经
   通过，项目负责人确认生成候选此前已经导入目标 OneNet 产品。后续配置、皮重、投递和
   清运测试继续核对各自实际使用的字段，但不再设置重复导入动作；早期
   `docs/iot/onenet-thing-model.json` 仍不得作为当前导入源。
