...
> H-02 验收为 `done`。
>
> 2026-08-29 增量结论：全新腾讯云 CDB 正式空库已完成 V1～V60、最小权限账号和
> 初始超级管理员供应，旧业务数据未导入；当前后端仍连接旧 V59 数据库，COS 和真实
> 入口均未切换。详细状态见第 11 节。
...
或转账依赖后，这两条模拟数据已在同一事务中精确删除，正式库恢复为“迁移静态数据 + 初始
...
- 当前 V59 后端受 epoch 门禁约束，不能直接连接 V60 新库。切换前必须从当前代码构建并
