...
UTC `16:31` 再次通过15分钟短期远程维护会话执行只读核对。设备热点页面停在
`CLOUD_DECISION_AND_AUTHORIZATION / WAITING_CLOUD_DECISION`，验收信息确认子步骤已经完成，
“本次验收的封存授权”仍为等待；设备本地对应验收事件已经同时取得OneNet接受时间和后台确认
时间。生产数据库中最近四份证据均为`FAILED / SENSOR_SELF_TEST_FAILED`，其余持久存储、可信
时间、配置、MCU通信、双摄和照片上传回读均通过；设备资产保持`FAILED`且验收代次为0。后台
只为`acceptance_status=PASSED`且代次大于0的资产创建`AUTHORIZE_FACTORY_SEAL`可靠任务，因此
本次没有封存授权记录和下行任务，不是队列积压或OneNet下行超时。

同一设备随后上报的运行快照进一步确认称重健康为`OK`、烟感健康为`OK`，但配置为超声波的满溢
检测有效样本数为0、距离值为空、采样依据为`NOT_SAMPLED`。这与验收证据中的
`sensorsHealthy=false`及前述`fullnessReadStatus=UNAVAILABLE`一致。现场必须先处理满溢传感器、
接线或采样链路，再由新一轮验收形成通过结论；不能人工补造封存授权。

同时确认一个展示缺陷：v43热点投影只读取“验收事件是否收到后台确认”和“本机是否收到封存
授权”，没有接收后台`FAILED`结论。于是后台因验收失败而正确拒绝签发授权时，热点仍把该阶段
显示为`ACTIVE/等待`，不会自行显示“验收未通过”。该展示问题与真实满溢传感器阻断相互独立，
后续设备版本需要为后台验收结论增加可靠下行事实或采用不会把“没有授权”解释为持续处理中的
明确失败投影。诊断会话结束后已主动关闭，服务器端口22011租约释放，
`leaseCleanupPending=false`；未重启设备服务、操作机构或修改设备业务记录。
