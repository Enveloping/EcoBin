...
项目负责人随后要求增加后端允许版本。生产服务器`VM-0-16-ubuntu`的
...
本次**没有写入或覆盖TF卡，也没有做写后回读、冷启动或真实HIL**。后台认可只允许服务器接收
该运行时版本的上报，不能替代写卡和现场验证。
...
本轮没有获得写卡授权，因此没有选择磁盘、覆盖TF卡或做全范围回读。当前已写卡的v42不因v43
构建或后端认可而发生变化。后续如需使用v43，必须另行明确写卡目标，重新核对磁盘身份、容量和
镜像摘要，完成整镜像写入与全范围回读后再装卡冷启动。
