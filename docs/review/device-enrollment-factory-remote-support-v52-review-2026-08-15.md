...
> 当前状态：7 项问题均已修复并完成自动化回归；真实 OneNet、微信、OpenSSH 和断电试点仍按部署手册执行
> 权威设计：[V52 设备自注册、厂家初始袋与按需远程维护](../architecture/device-enrollment-factory-acceptance-remote-support-v52.md)

## 1. 结论与已确认取舍

本轮发现 4 项 P1 和 3 项 P2。P1 会使生产证书签发、厂家实体扫码门槛、设备重启重连或
四端口容量在正常场景下失效；P2 会破坏验收证据代次、遗留无主远程入口，或使设计文档
与既有初始皮重状态机矛盾。
