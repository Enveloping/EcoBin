...
状态：**S3 连续离线占用释放已完成本地候选实现与验证，尚未部署。** 本文前半保留实施前只读审计，末尾第 8 节记录实际落地结果。业务决定没有改变；本地通过不表示线上数据库、设备或现场已经更新。
...
以下是实施前定义的验收矩阵。第 8 节逐项说明本批已经取得的证据与仍保留的边界；未在第 8 节列为通过的项目仍不能据此表宣称完成。关键迁移、事务、锁和真实结果接收使用隔离 MySQL 8.4，不能以 H2 或只匹配 SQL 字符串替代。
...
本节保存的是实施前审计结论；实际实现与验证见下一节。整个 S3 批次始终没有部署、SSH、串口、GPIO 或固件烧录。
...
- 新增前向迁移 `V80__offline_occupancy_release.sql`。连接事实增加 `offline_since_at`；投递会话和清运操作分别增加 `offline_occupancy_released_at`。历史 OFFLINE 设备从迁移时的数据库 UTC 时间保守起算，不猜测过去的离线起点。领域表仍为 137 张。
- 可信 OneNet 连接事件第一次转为 OFFLINE 时保存后台接收时间；连续重复 OFFLINE 保留第一次时间；ONLINE 清空；现有乱序拒绝规则保持。普通设备消息、下发失败和香橙派时钟不参与计时。
- 投递和清运各有独立扫描器，默认每 10 秒扫描，只有 `offline_since_at < UTC_TIMESTAMP(3) - 600秒` 才进入候选。每个原业务使用独立短事务，重新锁定资产、连接事实、原业务和准确占用；释放标记与精确删除原占用同事务提交，任一步失败整体回滚。
- 释放器不修改原业务状态、结束时间、可靠任务、用户归属、当前袋、皮重或 `CLEAN_RESERVED` 新袋预留，也不清香橙派本地工作槽。
- 原用户的活跃投递查询排除已经释放占用的旧会话，因此可以去其他在线设备。原设备的投递和清运准入仍检查未结束的待补报业务；设备选择页也把该事实显示为整机忙碌，避免先显示可用、点击后才拒绝。
- 原投递/清运查询返回释放时间。小程序显示“原结果待补报”和“可使用其他在线设备”，不显示为业务失败或完成。
- 正常完成、真实未开门/未解锁失败以及从未派发授权过期等出口仅在原业务已有释放标记且当前没有任何占用行时接受精确删除零行。没有标记仍必须存在完全匹配的原占用；若出现其他业务占用，旧结果不能删除或覆盖它。清运最终结果的旧袋、新袋预留和重量校验没有放宽。
- 投递结果长期不返回时，既有“确认从未开始”和“未知物理结果异常隔离”两种人工收口也采用相同占用规则；释放后仍能结束原待补报业务，但只有操作人原有确认条件、可靠任务和现场安全条件全部满足时才允许，异常隔离仍只保存问题证据且业务价值为 `NONE`。
- 数据库最小版本推进到 V80，运行账号权限清单推进到 41，只增加三个新字段所需的列级 UPDATE 权限；没有授予整表 UPDATE。

### 8.2 已取得的验证证据

- 确定性单元测试覆盖 599、精确 600、601 秒边界，连续 OFFLINE、ONLINE 重置、缺少起点、扫描 SQL 与准入/展示 SQL。精确 600 秒为不释放。
- 一次性、仅回环地址的 MySQL 8.4.10 事务测试 3 项通过：599 秒不释放、601 秒释放；注入占用删除失败时释放标记与占用一起回滚；清运释放后状态仍为 `IN_PROGRESS`。测试容器已按标签核验并删除。
- 同一批完整迁移后的真实 MySQL 投递/清运中位数事务套件共 105 项运行，除最初事务测试使用真实时钟写“刚好 600 秒”产生的测试竞态外，其余 104 项通过；将真实时钟用例改为 599/601 后，专门的 3 项事务测试全绿。精确 600 秒继续由同一个 `now` 的确定性单元测试覆盖，不改变生产严格比较。
- 完整 V1→V80 迁移、137 张领域表、运行权限清单 41、应用 V80 启动门槛和可靠问题事务已在另一只一次性 MySQL 8.4.10 容器中通过；H02 权限契约通过，容器已清理。
- Java 21 全模块 `install -DskipTests` 通过；recycling 模块 171 项全绿；device 模块 334 项中本批及其他 333 项通过，唯一错误仍是既有业务发布包固定白名单尚未包含此前 S1/S2 的新硬件文件。该错误属于 S4 发布清单，不是本次离线事务失败，也不能隐去为全绿。
- OpenAPI 生成一致性、小程序 TypeScript 检查和 Web 生产构建通过。Web 构建仅保留既有第三方循环分块和大分块警告。

### 8.3 仍保留的边界

- 尚未做释放与 ONLINE、正常结果和新开始请求的专门多线程并发压力测试；当前通过锁后重查、精确行数和事务回滚保证正确性，可能的数据库死锁由下一扫描周期重试，不把它写成已经完成 T14。
- 本批没有清空或改写香橙派本地待补报槽，也没有部署验证 T17 的真实断网重连。既有本地持久化与精确确认保护保持，现场组合验证留到 S4。
- S3 只解决连续离线占用释放与迟到原结果。人工解除控制通信阻断、清运中断人工袋确认、发布包白名单、完整固件/热点页面和现场交付仍属于后续阶段。
- 未连接共享或线上数据库，未部署后端，未更新 OneNet，未连接香橙派，未烧录 MCU/HMI，也未操作真实机构。
