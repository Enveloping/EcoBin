...
| `ops_task_attempt` / O | operations 所有；attempt UUID、任务内尝试序号、租约 token | 调用边界、接管和技术结果各从空补一次；技术结果不冒充业务终态；外部技术请求号只用于诊断关联 | `lease_until, external_call_may_have_started_at, reclaimed_at, result_recorded_at, action_kind, technical_result, request_sha256, response_sha256, http_status, external_api_error_code, external_request_id, duration_ms, redacted_diagnostic` |
