...
>
> 2026-08-25 前向修订：微信普通授权查单可能不返回 `create_time`。`channel_created_at`
> 只保存微信实际返回的渠道时间；缺失时保持为空。24 小时期限使用外调前已经持久化的
> `submitted_at`，历史缺失时保守使用不可变的 `created_at`，不得用恢复查单或回调时间补造。
...
3. 微信受理后保存 `packageInfo` 和稳定的 24 小时确认截止时间；微信实际返回 `create_time` 时才保存首次渠道创建时间，否则保持为空并按上述后端稳定时间锚点计算期限。同一商户授权单号后续返回不同的非空 `create_time` 时只保存观察、主投影保持首次值并进入独立证据冲突；后续缺失该可选字段不能解除冲突，只有再次返回首次值才能自动解除。`CLOSED` 仍可收敛，但未获相同时间佐证时保留对账问题。小程序使用原 AppID 调起微信官方授权页。前端返回不能把授权改为 `ACTIVE`。
...
| 草案 ID | 标题 | 类型 | 依赖 | 验收结果 |
|---|---|---|---|---|
| FND-F01 | 钱包、机构账户与不可变双账本 tracer | AFK | 8 模块、V6/V7 | 空钱包初始化、双投影/明细和真实 MySQL 回滚成立。 |
