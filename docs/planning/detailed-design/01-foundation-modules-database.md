...
>
> 状态：**已批准；DD-004 已确认并写回 I-051/I-053；H-01、F-01、F-02、F-04 已完成，V5～V10、epoch guard 与成对切换尚未实施**
>
> 审查日期：2026-07-23
>
> 适用基线：A-006～A-010、D-001～D-045、I-051～I-055
...
| DD-002 | 新应用只扫描独立的 `db/p0-migration`，旧 `db/migration` 只随旧恢复制品保留；新旧迁移纪元不得被同一应用发现。 |
| DD-003 | 运行应用关闭自动 Flyway，使用 `ecobin_app`；V1～V10 只由一次性迁移作业使用 `ecobin_schema_owner` 执行。 |
...
- F-01 已建立九个目标模块，F-02 已把旧 system 行为迁入 identity 并使 system 退出；
  当前是九个目标模块加一个 legacy business 模块的 10 子模块过渡 reactor。
- 首次注册同事务参与端口、不可伪造 FK 构造引用和可信会话上下文已经实施并通过 77 项
  Java 测试；F-03 仍负责最后的 business 边界迁移。
- F-04 已在独立 `db/p0-migration` 实现 V1～V4 共 30 张表，并在 MySQL 8.4.10 的两个
  全新空库通过严格安装、结构一致性、组织隔离、约束反例和重装拒绝验证。
- 旧应用和旧 V1～V14 运行库尚未切换；V5～V10、数据库身份、epoch guard、正式 seed
  和成对切换仍按后续任务推进。
