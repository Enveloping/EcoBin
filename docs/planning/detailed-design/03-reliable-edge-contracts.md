...
>
> 状态：**已批准；F-08 已实施、复审并完成，F-11 实施中；F-10 软件已完成，STATE_SNAPSHOT 等线级数值仍需 MCU 负责人 HITL 与跨端联调收口**
>
> 审查日期：2026-07-24
>
...
| DD-010 | 正式切换一次性禁用旧 D1 和临时 AA/BB/CC/DD；不保留生产双解析器、QoS 0、内存作业或 `/tmp` 照片降级路径。 |
...
- Pulsar TLS 允许不安全连接；
- 投递临时 AA/BB/CC/DD 没有 CRC、独立 ACK 或命令身份，清运仍保留 D1 文本链；
- 设备独占只存在于投递进程锁，清运不共享；
...
4. 每个 `PORT` 包含同一快照/boot、`partIndex/partCount`、唯一投口号和投递门、重量、红外、烟雾、故障状态；清运门只能报告电磁阀通断推定状态及人工确认，不伪造门磁。
...
6. 投递门门磁/执行器、清运电磁阀、称重、红外、烟雾的状态/健康/故障映射；清运电磁阀通断只能推定门状态；
...
| EDG-02 | MCU UART 1.0 基础层 | HITL | EDG-01 | HELLO、CRC、ACK、重复命令和事件持久化真机通过。 |
| EDG-03 | 香橙派 SQLite 与恢复骨架 | AFK | 无 | 强杀进程后命令、outbox、照片、确认和序号不丢。 |
| EDG-04 | 香橙派 UART 1.0 与快照组装 | AFK + HITL 联调 | EDG-01～03 | 噪声、拆粘包、重复、重启不重复动作。 |
...
| EDG-10 | 旧协议切断与人工验收 | HITL | EDG-01～09 | D1/AA/BB/CC/DD 不触发动作，真机证据完整。 |
...
- UART 无 CRC/ACK/业务身份，或用新旧双解析器作为生产回退；
- 重量失败上报 0、负重量截断为 0；
- 投递和清运使用不同整机锁；
