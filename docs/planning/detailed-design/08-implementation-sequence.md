...
>
> 状态：**29 项任务已批准并发布；H-01、F-01、F-02、F-03、F-04、F-05、F-06 已完成，F-08 已实施并进入评审，F-11 实施中，F-09 与 H-02 已 ready 但仍须分别授权，F-10 软件已完成并等待 MCU/联调验收**
>
> 草案日期：2026-07-24
>
...
- 83 张目标表中的 V1～V5 共 54 张已实施并通过 MySQL 8.4 双空库验证，V6～V10 共 29 张仍待实施；
- 中心可靠任务尚未实施；
...
此外，后端/数据库关键链仍需串行穿过剩余目标库和可靠任务基础；模块边界、DD-004 和复合投递事务所有权已经确认，剩余设计外检查点是 UART HITL、实际人员容量和微信外部就绪。正式任务虽然已经补充角色 owner 与初始 effort range，但实际负责人尚未指派，估算也尚未用首批完成数据校准，因此 G1 仍不能作为可承诺日期。
...
而为 `blocked`，F-08 为 `in-review`、F-11 为 `in-progress`；其余任务保持
`blocked`。当前共 `done` 7、`ready` 2、`in-progress` 1、`in-review` 1、
`blocked` 18。
