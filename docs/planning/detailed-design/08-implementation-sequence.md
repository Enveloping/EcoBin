...
- **类型**：Mixed（AFK + 项目负责人适配边界确认）
- **Blocked by**：无
- **覆盖**：场景 3、4、6、7、11、12
- **结果**：形成机器可读 OneNet 命令/事件/确认及规范 UART Registry；投递只使用 session 级开始/最终完成，清运区分电磁阀解锁与人工关门确认。现有固定帧单片机由 F-11 显式适配，不以原生实现 UART 1.0 作为本任务完成门。
- **验收**：
  - Java 21/Python 3.11/C 可使用同一数值与样本；
  - 包含 STATE_SNAPSHOT 应用级分段；
  - 旧 D1/AA/BB/CC/DD 不在正式注册表；
  - 投递中间轮次不进入 OneNet，清运电磁阀状态不冒充门磁；
  - 固定帧适配不产生第二套 OneNet/后端业务协议，也不自动回退。
...
- **Blocked by**：F-10
- **覆盖**：场景 3、4、6、7、11、12
- **结果**：EdgeStore、command inbox、work slot、event/photo outbox、业务确认、显式 MCU 协议适配和启动恢复。
- **验收**：
  - 强杀进程不丢命令/序号/事件/照片/确认；
  - MQTT QoS 1 但仍等待业务确认；
  - COS 临时凭证不落盘/日志；
  - `uart-v1` 以 Stub 验证重启先完整 QUERY_STATE；固定帧模式验证重启不恢复或重放
    无法核实的旧物理命令；
  - 与真实固定帧 MCU 的通过条件留在 H-03/V-03，不用 Stub 冒充联调。
...
- **验收**：
  ...
  - 再次解锁仍为原操作；
  - 超时只允许原人恢复；
  - 清运员完成确认直接形成记录，不存在审核或奖励；可靠复算重量作为初始有效值，有 `clean.edit` 权限者可直接设置/清空当前有效重量并留痕；
  - 实体袋可重复使用且无生命周期状态。
  - 电磁阀通断不能推定物理门位，门位保持 `UNKNOWN`，人工关门确认独立保存；
    `SAFE_CLOSE` 不作用于清运门。
...
- **用户故事**：用户先完成一次免确认收款授权，再手动提现；审核员处理后 Fake 微信按授权自动收款并正确结算，平台处理 NOT_ENOUGH。
- **覆盖场景**：8～10、12（软件部分）
- **结果**：授权创建/查单、配置、双冻结、审核/渠道前终止、固定原单、授权后自动收款、历史确认页兼容、三终态、闸门和客户端贯通；用户不提供取消提现。
- **验收**：
  - 同钱包单活动提现；
  - 授权只有可信 `TAKING_EFFECT` 才开放新提现，创建和提交各复核一次；
  - 审核通过不等于成功；
...
- **覆盖场景**：3、4、6、7、11、12
- **输出**：
  - 固件版本、接线、串口参数和 AA/BB/EE/DD/EF 原始抓包；
  - 投递 session/本地继续、清运电磁阀自动断电与人工关门确认；
  - 前后重量、满溢原始观测和规范业务事件对照；
  - 重启、迟到/重复结果、串口断开和无法表达事实保持未知的真机证据。
...
- **负责人**：项目负责人/指定测试用户
- **覆盖场景**：8、12
- **输出**：一次真实免确认收款授权、授权查单证据、授权后真实转账受理、无需逐笔确认的微信零钱到账、原 `outAuthorizationNo`、`authorizationId`、`outBillNo` 和双账本一致。
...
| 边缘/设备 | F-10/F-11、V-03/V-04/V-06/V-07/V-08 设备侧 | 以冻结规范模型衔接云端；现有 MCU 通过显式固定帧适配，不直接改固件 |
...
- 当前 8 模块 reactor 已收口，旧 common/system/business 均已退出，跨模块边界门禁已建立；
- V1～V21 已实施并通过 MySQL 权威验证；V19 已增加投递配置管理，V20 已把清运审核结构前向替换为直接修改及变更留痕结构，V21 已补齐无门磁正常清运事实；F-07 已完成
...
固定帧真机 HITL、实际人员容量和微信外部就绪。正式任务虽然已经补充角色 owner 与初始
...
| G1 | 八模块、新库 V1～V10、可靠任务和空库 Fake 新栈共同可运行 | 初始关键链约 14～24 person-days；待实际负责人和首批吞吐校准，不承诺 7 月 30 日 |
...
7. 项目负责人已授权并完成 H-01、F-01、F-02、F-03、F-04、F-05、F-06；后续任务
   不继承这次授权。
