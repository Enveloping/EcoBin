...
>
> 状态：**29 项任务已批准并发布；H-01、F-01、F-02、F-04 已完成，F-03/F-05/F-09 已 ready 但未授权，F-10 软件已完成并等待 MCU/联调验收**
>
> 草案日期：2026-07-24
>
...
- 目标 9 模块已进入 reactor，旧 system 已由 F-02 迁移退出，仅 business 待 F-03 收口；
- 83 张目标表中的 V1～V4 共 30 张已实施并通过 MySQL 8.4 双空库验证，V5～V10 共 53 张仍待实施；
- 中心可靠任务尚未实施；
- F-10 OneNet/UART 机器契约的软件生成物已完成，香橙派目标 SQLite/UART 可靠链仍待 F-11；
- MCU UART 1.0 需要另一负责人并行；
- 微信支付/商家转账不能联调。

因此，“2026-07-30 前完整通过 12 个 M0 场景”不是当前条件下可信的承诺。1～2 天弹性也不能替代微信外部就绪或高风险并发/断电/真机验收。

此外，后端/数据库关键链仍需串行穿过模块边界、目标库和可靠任务基础；DD-004 和复合投递事务所有权已经确认，剩余设计外检查点是 UART HITL、实际人员容量和微信外部就绪。正式任务虽然已经补充角色 owner 与初始 effort range，但实际负责人尚未指派，估算也尚未用首批完成数据校准，因此 G1 仍不能作为可承诺日期。
