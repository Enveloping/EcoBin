...
>
> 状态：**已批准；2026-08-06 已同步 D-046/I-056 的一次性授权与授权后自动收款验收边界**
>
> 审查日期：2026-07-24
>
> 适用基线：A-016～A-020、D-026～D-046、I-001～I-015、I-031～I-040、I-051～I-056、M0 场景 1～12
...
清运记录页面将“修改”作为普通直接保存动作：有 `clean.edit` 时展示当前有效重量和备注编辑器，提交 `expectedVersion` 与必填原因；成功后刷新为新版本，不显示待审核、提交审核、通过或驳回按钮。修改历史独立展示前后值和操作者，不命名为审核记录。
...
它不替代订单 revision、清运记录修改留痕、资金明细、设备事件或微信观察。
...
| 8 | 手动提现成功 | 用户先完成一次真实免确认授权；新提现双冻结、人工审核后无需逐笔确认即真实到账，授权单号、转账单号和双账本一致 | 授权运行时代码与真实微信验收待完成 |
...
| CLI-02 | Web 身份、设备与投递审核工作台 | AFK | 对应后端切片 | 人员/设备/投递审核/清运记录直接修改页面按能力和真实状态工作。 |
