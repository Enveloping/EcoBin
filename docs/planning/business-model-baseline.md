...
> 整理日期：2026-07-21
> 最近同步：2026-07-24，投递收口为会话一单/边缘本地多轮；PDD-001 仅保留首次会话建立的跨域协调，不再包含继续投递云端事务
...
| 提现与渠道 | 提现单、双侧冻结、用户确认收款和微信转账结果 | 用户钱包、机构资金、微信支付 |
...
| 清运记录 | 清运完成事件形成的换袋及称重事实 | 不因后续审核而回滚真实换袋 |
...
- 清运门是通电解锁并自动弹开的电磁阀门，门扇由清运员手动关闭；首期没有门磁，系统只以电磁阀通断推定门开关。
...
- 清运记录审核只认定净重量，不回滚已经发生的袋绑定和重量基准变化。
...
- 提现单负责用户申请、机构审核、双侧资金冻结、取消边界和最终业务结算。
- 微信转账单负责 `out_bill_no`、`transfer_bill_no`、渠道状态、失败原因、收款确认参数及回调查单结果。
...
“微信处理中”内部映射微信的 `ACCEPTED`、`PROCESSING`、`WAIT_USER_CONFIRM`、`TRANSFERING` 和 `CANCELING`，但这些都不是本地资金终态。

- 提现单创建时同时冻结用户余额和机构额度。
- “已取消（本地）”只表示仍处于待审核、尚未审核通过且未提交微信的手动提现被用户取消；“已撤销”只表示已经提交微信的转账最终到达渠道 `CANCELLED`，两者不是同一状态。
- 驳回、允许的本地取消、微信 `FAIL` 或 `CANCELLED` 同时释放两侧冻结。
...
结束用户等待、释放整机占位和允许原投口开始下一会话是三个事实。只有最终投递门安全、旧开门动作不会再执行、唯一完成结果已可靠保存且会话结束后容量 gate 已建立，才能释放或重新开放对应范围。普通满溢检测只在整场结束后进行并影响下一会话，不参与本会话内继续投递判断。
...
- 首期清运门没有门磁：电磁阀通电解锁且门自动弹开，门扇由清运员手动关闭；系统只能用阀通断推定门状态，不能获得真实门位证明。
...
- 完成事务立即反映真实换袋、当前袋码和新重量基准状态；只有可靠合法重量才建立有效基准，明确称重故障则置为无效并保持阻断。审核只认定清运净重量和统计，不回滚物理事实。
...
- 符合既定边界的手动提现可以在提交微信前取消；自动提现不可由用户取消。
...
- `NOT_ENOUGH`、长时间未结算、等待用户确认和系统错误是渠道处理中附带的状态或标记，不是失败终态。
...
- 提现尚未越过微信渠道提交边界：保留两侧冻结，并增加“负余额暂停”标记，禁止提交微信；后续已确认的钱包资金变动使可提现余额恢复为大于或等于 0 时，系统解除该标记，原提现按原审核进度继续。处于待审核的提现仍可按原规则驳回，符合取消边界的手动提现仍可取消。
...
4. **清运第一次可能解锁是不可逆分界点**：电磁阀可能成功通电前可以结束并释放预留；其后只能由原清运员继续或确认完成，阀断电不能证明门扇关闭，`SAFE_CLOSE` 不适用，最终完成仍记录真实换袋且审核不回滚物理事实。
...
| 后端接收清运完成 | 操作身份、旧袋与预留新袋一致；同一操作尚未完成 | 幂等创建一笔清运记录；结束操作；写入换袋历史；更新当前袋、有效重量基准和满溢/故障状态 | 本地整体失败则不确认，设备使用原事件重试 |
| 清运审核通过 | 按配置需要审核或存在系统异常；操作者有权限 | 保存最终认定净重量及审核记录；只影响记录认定和统计 | 不回滚袋绑定、当前重量基准或已经发生的物理事实 |
...
| 审核驳回或允许的本地取消 | 尚未提交微信且满足已确认的驳回/取消边界 | 提现终止；用户和机构冻结同时释放且各追加一次明细 | 任一释放失败则整体不成立 |
| 客服渠道前终止 | 提现已经审核通过但仍待提交；不存在微信转账单；操作者有权限 | 形成渠道前终止事实；用户和机构冻结同时释放；活动占位释放；记录完整审计 | 只要转账单存在即禁止本地终止，改走原单查单、续办或微信撤销 |
...
| 清运结果已接收 | 清运操作 ID | 一笔清运记录、换袋历史和投口当前状态 | 返回原记录，不重复换袋 |
| 清运记录已认定 | 清运记录 ID + 认定版本 | 最终认定净重量和审核记录 | 返回原认定，不重复改变统计 |
...
| 重复设备事件会不会重复建单？ | 不会，投递会话 ID 单独约束投递结果与订单最多一个，完成事件 ID 只做去重/冲突检测；清运操作 ID 单独约束清运结果 |
| 延迟投递会不会错归后来用户？ | 不会，归属依赖原会话冻结快照，不依赖到达时的当前会话 |
| 清运审核会不会回滚真实换袋？ | 不会，物理换袋事实与净重量认定分离 |
...
2026-07-23，项目负责人确认接口 I-026～I-030 的可恢复清运、单一完成事件、袋追溯和真实称重原则；其中“实际门扇打开/关闭可被设备证明”的隐含前提已由 2026-07-24 的电磁阀通断推定、人工关门与清运员最终确认边界修正。
