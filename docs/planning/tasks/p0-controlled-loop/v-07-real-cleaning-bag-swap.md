...
在清运业务启用前新增 `V20` 前向迁移：删除 `rec_organization_clean_config.review_mode` 及其旧 CHECK，删除 `rec_clean_record` 的审核 CHECK、待审索引、审核修订外键和审核/认定列，删除 `rec_clean_revision`；同时为清运记录增加当前有效重量/来源、记录备注、修改版本和更新时间，新增只追加 `rec_clean_record_change`，并建立 `clean.edit` 权限定义。V5/V8/V10 不得回改。迁移必须先断言不存在清运审核修订或依赖其人工认定值的业务数据；若发现历史审核数据则失败并转人工迁移设计，不能静默丢弃。V20 前后总表数都为 83，但历史审核表被修改留痕表替换。
...
- [x] V20 能在 V19 投递配置迁移后严格前向升级，保留清运操作、记录、原始/复算重量、异常、照片、袋、基准和检测事实；清运审核表、列、外键、CHECK 与待审索引全部消失，`rec_clean_record_change` 和 `clean.edit` 建立，V5/V8/V10 checksum 不变。
...
- 2026-08-01：按可信清运员决策删除清运审核、待审队列和奖励验收；清运完成直接形成记录。随后确认记录可由 `clean.edit` 直接修改并留痕，保存即生效；V20 改为以修改留痕表替换历史审核修订表，授权状态不变。
- 2026-08-01：已获明确实施授权并完成后端正常清运纵向切片：V20/V21、清运员准备、OneNet 下发与完成事件、原子换袋/新基准/清运后检测、本人及 Web 记录查询、直接修改和只追加历史均通过 MySQL 权威用例。任务仍保持 blocked，原因是再次解锁、超时原人恢复以及香橙派/MCU 真机链路尚未实施和验收。
- 2026-08-09：增加 Web 清运操作列表/详情和 V44 查询索引；可信边缘命令阶段会单调投影为 `EDGE_SAVED`、`IN_PROGRESS`、`RECOVERY_REQUIRED` 或 `PRE_UNLOCK_ENDED`，固定帧执行期重启继续使用既有 `ABORTED` 与联锁语义。小程序统一七种正式状态并阻止测试预览读取真实袋码追溯数据。再次解锁、主动解锁前结束、原人恢复和六类设备筛选仍未实施，任务状态不变。
