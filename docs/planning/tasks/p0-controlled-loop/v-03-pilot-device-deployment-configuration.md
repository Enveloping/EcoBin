...
> [!WARNING]
> 本任务正文发布早于 2026-08-07 的 V36 / D-047 / I-057 永久归属裁决以及当前
> fixed-frame 配置语义，现有“部署实例、部署码、经营开关、MCU staging/commit 双证明”
> 不能再直接实施。正式领取前必须先把任务重定为“资产一次性永久分配租户和机构、香橙派
> 可靠保存并激活配置即可形成当前阶段 `APPLIED`、按当前权威事实自动计算业务资格”，并
> 同步验收标准和依赖。本提示不改变 `status: blocked` 或
> `implementation_authorized: false`。
>
> 2026-08-29 当前现场已在现用测试数据库/COS 上走完已经实现的永久归属和自动启用流程：
> 平台已分配测试租户，租户已分配测试机构，配置 v1 已精确 `APPLIED`，初始袋皮重已完成，
> 当前投递准入事实满足。这项现场结果不需要等待生产 CDB/COS 迁移，也不表示按下方旧正文
> 领取或完成 V-03；正式领取前仍须先重定任务基线。
...
- [ ] 只有最高期望配置精确 `APPLIED` 且投递门、协议、传感器、存储等安全条件成立时，
  才允许激活及开启经营；清运门仅有电磁阀通断和人工确认能力，物理门位保持 `UNKNOWN`。
- [ ] 投递与清运资格分别计算；满溢或袋/基准问题不得错误阻断一次可恢复清运。
- [ ] 只有应用仍为 `PENDING`、尚未边缘落盘且任务以明确下发前原因阻断时，重同步才复用原 application、command 和 task；`FAILED` 必须发布更高版本，不能用第二份同版本意图绕过失败。
...
- 设备调拨；
- MCU 固件实现本身；
- 旧 D1、旧业务处理或自动双协议兼容；当前已确定固定帧只能经 F-11 单一显式适配器接入；
- 以持续传感器心跳替代冻结业务检测；
