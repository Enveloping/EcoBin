...
- [ ] 平台管理员能创建、禁用租户，并通过明确审计的特权用例处理租户业务数据。
- [ ] Web 登录只需要登录名和密码，不要求租户编码；工作人员登录名在全部租户间唯一。
- [ ] 平台账号与租户工作人员使用独立入口和命名空间，认证错误不泄露账号是否存在。
- [ ] Web 会话只使用符合冻结属性的 `Secure + HttpOnly + SameSite=Lax` Cookie，JavaScript 不接触会话 Token。
- [ ] 登录及所有非安全方法执行 CSRF 校验，客户端不把 Web Bearer Token 写入 localStorage。
- [ ] 租户主体和机构负责人天然权限正确；总部及普通员工的多职责能力可配置。
- [ ] 两租户、两机构越权读写统一表现为不可见，客户端提交的 tenant、organization 或 role 不能扩大权限。
- [ ] 禁用账号、租户或机构，或修改密码、任职、权限后，相关会话立即失效。
- [ ] Web 完成租户、机构、员工、任职和授权的可操作闭环，并记录操作者、时间及必要前后值。
- [ ] 真实 MySQL 集成测试覆盖会话竞争、全局登录名唯一、租户拦截和机构级显式隔离。

## 阻塞与最早开始

本任务只有在 F-02、F-03、F-04、F-09 全部完成后才能开始并整体交付。发布本任务不代表已经授权修改代码。
