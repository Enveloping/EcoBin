...
> 本任务只交付可验证的软件与 Fake 渠道闭环。真实商家转账、确认页和微信零钱到账必须由 H-05 单独证明。

## 目标

让机构用户手动申请提现，审核员作出不可改金额的决定，系统双侧冻结用户和机构资金，并通过 Fake 微信适配器验证固定原单、渠道未知态、三种终态、`NOT_ENOUGH` 和恢复行为。客户端只显示后端权威状态。

## 要构建什么

- 实现机构手动提现配置、单活动提现槽、提现状态机和用户/机构双侧不可变资金明细。
- 按冻结锁序实现提现创建；任一前置不满足时不建单、不写失败记录、不改变余额。
- 实现审核通过/驳回、用户合法取消和客服渠道前终止；审核通过只进入
  `READY_TO_SUBMIT`。
- 在短事务中建立唯一微信转账行、固定 `outBillNo` 和不可变请求摘要，再由可靠任务在事务外调用 Fake 渠道。
- 统一归并提交响应、回调、查单、撤销和对账观察；只有
  `SUCCESS/FAIL/CANCELLED` 可以结算或释放双侧冻结。
- 实现 `WAIT_USER_CONFIRM` 安全视图、小程序确认入口和确认页返回后的后端轮询。
...
- [ ] Web 和小程序不把审核通过、微信受理或确认页调起显示为成功到账。
...
- 真实商家转账受理、真实确认页和微信零钱到账，由 H-05 验收。
- 自动提现、免审提现、免确认授权和自动恢复出款闸门。
