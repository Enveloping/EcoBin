...
1. 当前现场 Web/后端实际写入的 V60 数据库和现用 COS 都是测试环境。先在这里完成设备归属、配置、
   初始皮重、扫码、投递、清运和后续业务闭环；CDB/COS 迁移在测试完成后执行，并从迁移后
   才开始按生产环境对待。旧测试业务数据仍不迁移。
2. 当前 OneNet 目标产品已经导入 15 个服务、18 个事件、0 个属性、33 个功能点的候选；
   项目负责人确认此前只是文档落后，不再重复导入。
3. 指定设备当前机器验收为 `PASSED`、当前封存代次为 `SEALED`，并已永久分配启用中的测试
   租户和该租户下启用中的测试机构；两次不可变归属均已成功落库。
4. 机构分配事务已经自动建立 1 个投口、厂家初始袋占用、运行投影和配置 v1。设备精确报告
   `APPLIED` 后，初始空袋皮重也已自动完成；当前袋/皮重、投口启用与单价、OneNet 在线、
   设备空闲及互锁状态均满足投递准入。下一步由小程序用户扫描设备二维码，发起一次正常投递
   并核对唯一待审核订单。反向 SSH 只是按需维护手段，不是正常接入的必经步骤。
5. 过时的 V-03 任务正文仍须在正式领取实施前重定基线；本轮只验证已经实现的永久归属与
   自动启用流程，不据此宣称旧 V-03 任务完成。
...
| H-03 | [固定帧 MCU 线路与真机基础验收](h-03-fixed-frame-mcu-hil-acceptance.md) | `in-progress` | `human` | v8 写卡、P7/P8、联网注册、封存冷启、远程维护生命周期、OneNet 当前模型、永久归属、配置 v1 与初始皮重均已确认；当前测试下一步是小程序扫码投递 |
...
- 2026-07-27：项目负责人明确授权 Codex 在独立 worktree 推进 V-02，任务和 software
  ...
  当时共 `done` 13、`ready` 0、`in-progress` 2、`blocked` 14。
...
- 2026-07-28：F-11 固定帧适配与照片/COS 链路已合入。项目负责人确认保留完整
  ...
  真机操作仍未授权。当前共 `done` 14、`ready` 1、`in-progress` 1、`blocked` 13。
...
- 2026-08-29：H-03 的空白冷启动、Air780E、注册、P7/P8、单向封存、封存后冷启动和
  ...
  当前候选已复核为 15 服务 / 18 事件 / 33 个功能点；项目负责人确认该版本此前已经导入
  OneNet，先前待导入口径是文档落后。现用数据库与现用 COS 继续作为测试环境，生产 CDB/COS
  迁移推迟到测试完成以后。V-03 在重定基线并获得授权前仍保持 `blocked`。
- 2026-08-29：指定设备已完成测试租户和测试机构的永久归属；现场写入库核对为 V60。机构
  分配后自动建立 1 个投口、初始袋和配置 v1，配置及初始空袋皮重两个可靠任务均一次成功，
  配置在 application/runtime 两侧精确 `APPLIED`，当前袋 0 g 皮重有效且投口 `NOT_FULL`。
  全部投递准入事实满足，当前流程进入小程序扫码发起一次正常投递并核对待审核订单。
