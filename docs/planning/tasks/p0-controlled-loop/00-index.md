...
1. 当前后端连接的 V59 数据库和现用 COS 都是测试环境。先在这里完成设备归属、配置、
...
3. 指定设备当前机器验收为 `PASSED`、当前封存代次为 `SEALED`，并且尚未分配租户或机构。
   下一步由平台永久分配一个启用中的测试租户；然后用该租户主体或具备
   `device.assignment.manage` 权限的账号，永久分配该租户下一个启用中的测试机构。这两次
   写入在当前测试库内也不能修改、清空或调拨，操作前必须核对目标租户和机构代码；选错后
   按业务规则只能报废该资产，不能直接改归属。
4. 机构分配事务会自动建立投口、厂家初始袋占用、运行投影和配置 v1，并登记可靠
   `APPLY_CONFIGURATION` 任务。设备在线接收、可靠保存并报告 `APPLIED` 后，系统自动为
   各投口发起初始空袋皮重；配置、皮重及其他实时准入事实满足后才允许测试投递/清运。
   反向 SSH 只是按需维护手段，不是正常接入的必经步骤。
...
| H-03 | [固定帧 MCU 线路与真机基础验收](h-03-fixed-frame-mcu-hil-acceptance.md) | `in-progress` | `human` | v8 写卡、P7/P8、联网注册、封存冷启、远程维护生命周期及 OneNet 当前模型均已确认；当前测试下一步是永久分配测试租户/机构并观察自动配置和皮重 |
...
- 2026-08-29：H-03 的空白冷启动、Air780E、注册、P7/P8、单向封存、封存后冷启动和
  ...
  OneNet，先前待导入口径是文档落后。当前 V59 数据库与现用 COS 继续作为测试环境，下一步
  在当前库永久分配测试租户和机构并观察自动配置、皮重及业务准入；生产 CDB/COS 迁移推迟
  到测试完成以后。V-03 在重定基线并获得授权前仍保持 `blocked`。
