...
- [x] v10 空白启动的 P7 网页验收已完成；热点保留且没有自动注册的直接原因已在真实
  systemd 权限上下文中定位为 Chrony 控制目录权限错配。候选修复、精确结果码、journal
  状态转移和网页诊断均已完成，设备侧 Python 3.11 全量回归为
  `1018 passed, 46 skipped, 5 subtests passed`；权限矩阵未发现第二个已能触发的同类错配。
- [ ] 把当前候选原子热部署到正在运行的 v10，证明实际进程能力、`chronyc` 返回、可信
  时间、HTTPS 注册、OneNet 和运行时自动收敛；通过后再从锁定输入完整重建 v11。
...
- 2026-08-30：v10 空白启动完成 P7 后，Air780E 已枚举并形成 RNDIS 链路，但热点没有关闭、
  HTTPS 注册和 OneNet 注册没有发生。现场对比证明：`/run/chrony` 为
  ...
  返回 `501 Not authorised`。因此根因是服务无法穿越 Chrony 控制目录，不是模组、NTP
  可达性或 P7 状态。候选只给该长期服务增加 `CAP_DAC_OVERRIDE`，并新增 root 私有的本次
  ...
  下一步先在 v10 原子热部署并按真实服务权限验证自动收敛，再完整重建 v11。
