...
这里的“不能替代”只限定真实硬件物理质量与正式发布资格，不否定接入流程验证。按同一固定帧
协议产生相同语义数据后，真实 MCU 和模拟 MCU 共用唯一的 P7 验收、联网、校时、注册、OneNet
运行、P8 授权与封存实现；系统不得按数据来源选择不同的后续业务路径。模拟来源标记用于证据
追踪，不参与平台机器验收的通过/失败判定。当前阶段只验证流程时，可以据此确认流程节点通过，
无需同时取得真实 MCU、屏幕、传感器或执行机构质量结论；后者在真实硬件资格/HIL 阶段单独
签认。
