...
6. 设备离厂前证明 K1 已删除、MCU 为 revision 2、两个摄像头、投递/清运执行器和 MCU
   升级控制线路可用；
...
| 8 | BOOT0 接物理 7 号针；NRST 经 2N7002 开漏级接物理 11 号针 | 固化为 PC9/wPi 2 和 PC6/wPi 5；香橙派两路动作均为高有效，MOS 只负责拉低 NRST，不用推挽门驱动复位线 |
...
| 11 | 无需 SSH 的出厂预检 | 手机网页发起 revision 2、双摄、升级线路以及真实投递/清运动作测试；验收服务独占 UART/GPIO/摄像头 |
...
硬件仍必须具备 BOOT0 约 10 kΩ 下拉、BOOT1/PB2 固定下拉、NRST 上拉以及上述可靠开漏控制，
确保香橙派尚未启动或进程崩溃时 MCU 默认进入应用。软件设置不能替代这些失效安全电阻。
...
| `FACTORY_TEST_RUNNING` | 已原子建立唯一测试运行 | revision 2、F1、升级线路、双摄、投递和清运动作均产生本地结果 | 异常进入失败或恢复锁，不自动重发 AA/EE |
...
它们不会产生业务或云端事实，但门锁、电机、屏幕、按钮、称重、传感器和摄像头都是真实动作，
操作员必须按真实设备的夹伤、坠落和误开门风险对待。
...
| 升级线路 | F2 准备、BOOT0/NRST、ROM Bootloader 非写入探测、复位回原身份和 F1 | 否 | 进入可恢复失败，禁止离厂 |
...
身份并通过 F1，之后才能解除本地预检维护锁。恢复失败进入 `RECOVERY_REQUIRED`，不能直接
启动业务。
...
- MCU F3 为 revision 2，F1 健康，BOOT0/NRST 探测后回到原应用身份；
- 两个摄像头路径和人工内外确认通过；
...
1. 锁定的 Debian 12 / Linux 6.1 官方基础镜像在目标 Orange Pi Zero 3 批次和标称 32 GB TF
   卡上可启动、可扩容；
2. BOOT0 下拉、BOOT1 下拉以及 2N7002 Gate 下拉/NRST 上拉和电容已经落板；物理 11 号针经
   开漏级控制 NRST 的方案已经确认，这里只做仪表复核，不再等待产品决策；
