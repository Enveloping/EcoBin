...
> 整理日期：2026-07-22
> 最近确认：2026-08-06，项目负责人确认提现采用“先授权、后自动收款”；A-001～A-020 的模块/部署职责仍全部确认
...
2026-07-26 实施进展：reactor 当时由 F-03 收口为包含 `ecobin-common` 的九个模块；system 已由
F-02 迁入 identity 并退出，business 也已迁入 funds、recycling、operations 并退出。
2026-07-29 清理不可达旧栈后，`ecobin-common` 的五个旧栈专用类型与模块本身一并删除；
当前为八模块 reactor，通用 Web 技术契约位于 framework，业务值类型位于所有权模块的
`.api` 包。
独立 V1～V10 的 83 张历史表已由 F-04～F-06 验证；清运审核结构由 V20 前向替换为直接修改及变更留痕结构后，目标业务表数仍为 83。F-07 新运行制品已物理移除
Flyway 运行库和迁移脚本，
...
| 清运 | 仍是开门时创建记录、`cleanGross`/`cleanTare` 分段更新的旧流程 | 与已冻结的可恢复清运操作及单一 `cleanComplete` 不一致 |
...
| 清运与容量 | 完整清运操作状态机、清运记录及修改留痕、袋码预留/绑定历史、重量基准、满溢检测流程、快照和事件 | 准备/恢复/完成清运、直接修改记录当前有效业务数据、保存变更和异常、处理满溢检测并更新投口阻断状态；不提供清运审核或奖励流程 |
| 机构结算 | 充值单、机构出款账户、机构资金明细、免确认收款授权、提现单、微信转账单和全平台出款闸门 | 充值入账、授权状态收敛、双侧冻结编排、渠道状态收敛和资金不足恢复 |
...
| 香橙派 | 持久化投递会话/清运操作、投递本地轮次协调、首末净重与负重量异常锁存、会话结束后满溢判断、标准照片、COS/OneNet 队列和恢复 | 上传投递中间轮次数据、决定用户归属/投递审核金额/微信资金结果 |
...
17. `uart-v1` 任一端重启都先 HELLO、QUERY_STATE 并补收关键事件；投递门查询真实门控
    状态，MCU 启动代际改变时不得重放旧 START/OPEN，状态不一致时执行投递门安全关门并
    锁存恢复。`fixed-frame` 的 `F0/F1` 只查询重量、红外和烟感，没有通用作业/门状态
    查询；重启后只使用 SQLite 证据，将未知物理状态安全锁存并让未决工作失败或转人工
    恢复。两种模式都不得重放旧开门；首次可能解锁的
    清运不得自动执行 `SAFE_CLOSE`、声称门已关闭或完成。
18. 正式切换禁用旧 D1 和旧 gross/tare 清运路径；现有 MCU 的已确定固定帧只允许经
    香橙派单一显式适配模式接入，生产不运行自动探测、双解析或失败后回退。
19. 清运门电磁阀通断只表示锁输出，不能推定物理门位；无门磁时门位保持 `UNKNOWN`。
    第一次解锁命令可能成功即越过不可逆边界；再次解锁沿用原操作；只有清运员手动关门、
    最终稳定重量或明确终态称重故障可靠保存并明确确认后才能上报 `cleanComplete`。恢复中
    仅断电不能释放整机占位；断电且原清运员现场确认门扇关闭后可释放整机占位，但原投口、
    操作和袋预留继续锁定。
...
- 商家转账适配器：发起/查询免确认收款授权，使用有效授权提交/续办原 `out_bill_no`，查询转账、解析回调状态并分类渠道错误；历史逐笔确认参数仅用于既有在途单兼容收敛。

充值和转账共享证书、签名、回调安全等基础能力，但不共享业务状态机。回调、主动查单、人工查单和每日对账必须进入同一幂等结算用例，不能各自修改资金。

P0 所有机构共用公司的系统微信商户号；机构不是独立微信商户。机构归属来自 EcoBin 授权单、充值单和提现单的不可变快照。免确认授权由该机构已绑定系统商户号的 AppID 和对应 OpenID 发起；新提现固化并复核同一授权身份，不能根据回调临时改挂机构或收款人。

微信超时、频率限制、授权等待用户确认和 `NOT_ENOUGH` 都不是系统可以自造的失败终态。授权申请和转账分别使用稳定商户单号、可靠任务、可信回调/查单与不可变观察收敛；`NOT_ENOUGH` 更新同一个全平台出款闸门和聚合告警，不否定各机构本地账本。
...
4. 微信商户私钥、APIv3 密钥、OneNet 产品级 AccessKey 和 COS 平台凭证只从后端部署环境或秘密管理设施注入；OneNet 设备 Key 只配置在对应香橙派，后端不接收、保存或回显。平台级必需配置缺失时生产启动失败。机构 AppSecret 则由具有小程序配置权限的 Web 用例直接保存到 `iam_organization_miniapp.app_secret`，与 AppID 和版本在同一个短事务中提交；数据库及备份按秘密数据保护，日志和审计脱敏。某机构尚未配置时只阻止该机构小程序激活和登录，不导致整个服务启动失败。
...
- 受控人工动作：重新查单、重试可靠任务和确认平台补资恢复都走业务用例并记录操作者，不能直接改表；当前系统不提供主动微信撤销。
...
| 香橙派重启 | 先恢复 SQLite 本地证据。`uart-v1` 查询投递门真实状态和清运阀通断；`fixed-frame` 无查询能力，把物理状态标为未知并让未决工作失败或转人工恢复。两种模式都不得把旧结果归给新用户，也不得把清运阀断电当作门扇关闭/操作完成 |
...
## 16. 第二批：Maven 物理模块方案及后续收口

> 确认状态：**2026-07-22，A-006～A-010 全部确认并同意；2026-09-03 按已运行代码补记后续收口：删除不再承载有效契约的 `ecobin-common`，其余职责和依赖方向不变。**

### 16.1 当前八模块布局

设计冻结时曾计划从 6 个模块调整到 9 个模块。F-03 完成了该结构，但后续目标实现已经
替换了 common 中的旧统一响应、分页、异常、角色和常量；2026-07-29 删除不可达旧栈时，
`ecobin-common` 随之退出。当前保留以下 8 个模块：

| Maven 模块 | 物理职责 | 当前代码的主要来源 |
|---|---|---|
| `ecobin-framework` | Spring/Web/Security 技术骨架、通用 Web 响应与异常契约、可信执行上下文、租户/机构数据防线、MyBatis 与事务配置 | 当前 framework 中与具体外部平台无关的部分，以及已替代旧 common 响应/异常的技术契约 |
...
| `ecobin-module-recycling` | 投递订单及其审核纠错、完整清运状态机、清运记录与修改留痕、袋码、重量基准、满溢和 P0 回收查询 | 当前 Delivery/Clean/Statistics 中的 P0 部分 |
...
香橙派重启时先打开并校验 SQLite。`uart-v1` 恢复进行中作业并查询 MCU 状态，投递门按
真实门控状态决定安全关门或恢复；`fixed-frame` 不查询 MCU，把缺失状态保持为
`UNKNOWN/NOT_SAMPLED`，未决工作明确失败或转人工恢复。重启后绝不自动重新开门；首次
可能解锁的清运不得自动执行 `SAFE_CLOSE` 或完成，必须等待原清运员现场恢复。
...
| 日期 | 变更 |
|---|---|
| 2026-09-03 | 根据当前根 POM、模块边界门禁和 2026-07-29 的旧栈删除事实，将现行物理拓扑由原九模块更正为八模块：`ecobin-common` 退出，通用 Web 响应/异常归 framework，业务值类型归所有权模块 `.api`；其余依赖 DAG、事实所有权和事务规则不变。 |
| 2026-08-06 | D-046/I-056 将商家转账适配边界扩展为免确认授权创建/查单和授权后转账：授权成为 funds 所有的独立渠道事实，新提现创建与提交分别核对当前有效授权并固化身份；历史逐笔确认提现按原快照继续收敛。当前八模块依赖、单库事务、可靠任务和可信渠道终态结算原则不变。 |
...
| 2026-07-23 | 接口 I-026～I-030 同步清运与恢复：旧开门即创建记录/gross/tare 链路由可恢复操作和单一完成事件整体取代；清运换袋、基准和检测 gate 原子接管容量；工作人员小程序只读容量告警，真实重检/重测和精确安全恢复保持 Web 受控入口。 |
...
| 2026-07-27 | 现有 MCU 不再原生实现 UART 1.0；规范模型保持冻结，香橙派以单一显式固定帧适配器隔离线路差异，缺失能力不得伪造，旧 D1、自动探测、双解析和失败回退仍禁止。 |
