...
- 投递门的真实门控/门磁状态，称重、红外、烟雾健康，清运电磁阀的当前通断状态、
  独立的清运员关门确认，以及 recycling 模块提供的当前容量/满溢安全摘要；无门磁时
  清运门物理状态固定为 `UNKNOWN`；
- 当前整机占位安全摘要；
...
5. 香橙派与 MCU 在线，关键组件健康，全部投递门由真实传感器确认关闭，清运门电磁阀
   已断电；后者不能推定物理关门，清运门位仍为 `UNKNOWN`；
6. 不存在投递或清运整机占位；
...
- 开启要求部署已经 `ENABLED`，租户/机构有效，最高配置精确应用，协议兼容，设备在线，
  全部投递门状态已知且真实关闭，清运电磁阀断电，并且没有阻断安全锁或整机占位。
  清运门位在该判断中仍为 `UNKNOWN`，锁断电不能伪装成真实门状态。容量已满只会阻断
  对应投口投递，不改变整机经营意图。
