...
- `userDisplayName` 由后端确定性生成，例如 `金收宝用户-` 加机构用户公开 UUID 的末 8 位；不接受客户端昵称，避免超长、表情或重试时变化。
...
相同幂等键和相同请求返回原申请；不同请求冲突。已有 `ACTIVE` 授权时直接返回当前授权，不生成第二条。已有待确认申请时返回原申请及当前状态，不消耗微信允许的授权数量。
...
I-056 对应的 Controller、OpenAPI、渠道端口、可靠任务、通知入口、提现状态机、Web 展示和小程序授权流程已经在代码中实现；Fake 模式及真实 MySQL 约束路径已完成自动化验证。
