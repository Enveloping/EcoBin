...
- `AUTHORIZE_FACTORY_SEAL` 是非物理授权的窄例外：其 `expiresAt` 是设备首次可靠受理的截止点。只要完全相同的命令已在期限内写入 `command_inbox`，排队、进程重启恢复和同 UID 重投可以在截止后继续，但必须使用数据库保存的原始受理时间，不能信任命令载荷或调用方补传的时间。其他命令仍按各自执行期规则校验，物理命令、COS 临时凭证和远程维护期限不随此例外放宽。
...
**已确认：设备在后端授权的原作业前缀内自行生成对象 key 并直传 COS；照片失败不阻断投递订单、已完成清运记录或投递返现，后端通过独立可靠事实把四个固定槽位补齐或标为永久缺失。**
...
照片缺失、上传 pending 或后端暂未验证 URL 都不产生用户投递异常，不阻止投递订单、清运换袋、投递审核返现或业务确认。后端先为四个槽位建立状态，再异步暴露相机/网络/上传故障。
...
| `CLEAN_COMPLETE` | 规范物理结果、清运员人工关门/完成确认、电磁阀断电、清运物理门位 `UNKNOWN`、唯一已完成清运记录、袋交换/基准处理、四个照片槽、清运后检测 gate/必要采样任务、操作完成 | 满溢最终结论、照片上传完成；清运不存在审核或奖励，也不存在清运门门磁事实；后来直接修改记录不覆盖本设备证据 |
...
| `PHOTO_STATUS_REPORTED` | 原作业固定槽位已幂等关联为可用或永久缺失 | COS 再次查询、投递订单审核；清运记录不审核 |
