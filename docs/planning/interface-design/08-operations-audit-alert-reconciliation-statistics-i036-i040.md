...
>
> 状态：**I-036～I-038、I-040 后端接口已实现；I-039 经产品裁决暂缓**
>
> 说明：本文件定义平台技术处置、不可变操作审计、聚合告警、每日资金对账和最小运营概览的 P0 契约。完整投诉工单、外部告警通知、复杂经营报表、官方账单文件归档和财务导出继续留到 M1。

> [!IMPORTANT]
> 2026-08-05 实施裁决：本轮只交付 I-036 可靠任务/隔离治理、I-037 审计查询、I-038 告警查询与确认、I-040 最小运营概览的后端、OpenAPI 和生成类型。I-039 每日资金对账整体暂缓，因此当前不得发布 `reconciliation-runs`、`reconciliation-issues` 或对应人工处置 API。既有资金状态机发现明确矛盾时仍保留已有对账问题写入能力，但它不等于 I-039 查询、调度和处置闭环已经上线。

> [!NOTE]
> I-038 当前告警来源包含：平台出款余额不足、设备故障、投口满溢、可靠任务进入 `BLOCKED`。后台投影以来源事实为准：工作人员“确认已看到”不会解决告警；设备恢复、投口后续明确上报 `NOT_FULL`、可靠任务最终 `DONE/CANCELLED` 或出款闸门真实恢复后，告警才转为 `RESOLVED`。
...
尝试时间线按 `attemptNo DESC` 使用不透明游标，返回动作种类、技术结果分类、领取/外调可能开始/结果时间、HTTP 状态、外部 API 错误码、可空的外部技术请求号和限长诊断摘要。外部技术请求号只供运维人员关联 OneNet 等渠道日志，不是命令、事件或业务身份；接口不返回请求/响应原文，也不能把一次尝试的 `TECHNICAL_SUCCESS` 表述为订单、设备或微信业务成功。
