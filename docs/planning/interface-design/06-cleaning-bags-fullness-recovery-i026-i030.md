...
>
> 状态：**I-026～I-030 已确认；2026-08-05 已补当前清运员袋追溯与 V25 被动满溢查询接口**
>
...
> [!NOTE]
> 2026-08-05 当前实现：`aud=miniapp` 的当前机构清运员可扫描本机构袋码，读取按安装事件划分的使用周期、周期内投递单和脱敏用户/照片证据。周期归属只采用订单已冻结的袋、投口和后端会话创建时间，不按设备时钟或袋当前位置倒推。Web 与工作人员小程序的满溢接口展示 V25 `rec_fullness_state_change` 被动上报历史及当前容量投影；旧 `fullness-detections/fullness-events` 主动检测接口没有发布。
...
3. 清运门没有门磁或受控关门执行器。MCU 只能控制电磁阀：通电解锁后门自动弹出，
   断电只表示停止解锁；门扇由清运员手动关闭。接口不得按电磁阀通断推定门扇
   `OPEN/CLOSED`，物理门位保持 `UNKNOWN`。清运员关门确认作为独立人工事实保存，
   不得宣称这是物理门位检测。
...
- 已知整机或投递门安全故障、清运电磁阀仍通电、MCU 或边缘离线、协议不兼容、当前
  投口存在未终结清运、整机被占用或无法可靠保存新作业时必须阻止清运。电磁阀断电不能
  推定清运门关闭，物理门位仍为 `UNKNOWN`；存在上次解锁后的待恢复操作时仍不得创建
  新操作。称重在操作中途失败可以形成异常清运记录，但首次解锁前已经锁存为严重称重/
  本地存储故障时不能借清运绕过安全停用。
...
| 操作内断电或再次通电解锁 | `IN_PROGRESS` | `IN_PROGRESS` | 只追加锁输出真实观察并更新汇总，不覆盖首次解锁边界和旧袋快照；断电不改变物理门位 `UNKNOWN` |
| 已解锁操作超时或普通执行结果不确定 | `IN_PROGRESS` | `RECOVERY_REQUIRED` | 先保留原操作、整机保护和新袋预留；只有电磁阀断电且原清运员现场确认门扇关闭后，才可把保护降为原投口/操作/袋预留并释放整机占位。不得仅因断电自动释放或宣称门已关闭 |
| fixed-frame 执行期间收到可信香橙派重启事实 | `PREPARED/EDGE_SAVED/IN_PROGRESS/RECOVERY_REQUIRED` | `ABORTED` | 释放本次业务占用与新袋预留，但建立投口重启安全联锁；不得恢复或重放旧作业，人工确认现场安全后才能允许新作业 |
| 原清运员恢复命令被设备可靠接受 | `RECOVERY_REQUIRED` | `IN_PROGRESS` | 取得新执行窗口，不创建新操作 |
| 可信完成结果 | `IN_PROGRESS/RECOVERY_REQUIRED` | `COMPLETED` | 必须同时具备原操作身份、清运员最终确认与人工关门确认、电磁阀断电，以及最终稳定重量或明确终态称重故障；物理门位仍为 `UNKNOWN` |
| 可信证明首次解锁从未可能执行 | `PREPARED/EDGE_SAVED` | `PRE_UNLOCK_ENDED` | 释放预留和占位，不创建记录或照片槽 |

`PRE_UNLOCK_ENDED/COMPLETED/ABORTED` 是不可回退终态。中间观察乱序或缺失时，最终清运员确认、
独立人工关门确认、锁断电事实、最终稳定重量或明确称重故障，以及 `CLEAN_COMPLETE`
可以单调补进；没有可信证据的精确时间保持 `null`，不得用后端处理时间伪造设备动作。
查询中的 `cleanDoorPhysicalState` 始终为 `UNKNOWN`，人工确认单独返回。
...
- 只允许原清运员对首次解锁尚未可能执行的操作请求结束。请求不会仅凭客户端意愿写 `PRE_UNLOCK_ENDED`；服务端必须取消仍未外调的任务，或取得设备明确拒绝/停止且电磁阀从未通电、没有在途解锁动作的证据。
...
- 安全结束事务把操作置为 `PRE_UNLOCK_ENDED`，删除新袋当前预留、追加 `RESERVATION_RELEASED` 事件并释放本操作整机占位。它不创建清运记录、标准照片槽、基准或满溢检测。
...
1. 香橙派只在原 `operationUid` 仍为本地活动、当前锁输出已经断电、单调执行期限未到、
   MCU/电磁阀健康且操作未进入中心 `RECOVERY_REQUIRED` 时接受设备按钮事件。这里的
   断电只证明停止解锁，物理门位仍为 `UNKNOWN`。
...
- 操作一律进入 `RECOVERY_REQUIRED`，先保留原操作、整机 `CLEAN` 保护和新袋预留，
  等待原清运员现场恢复。电磁阀应进入断电安全输出，但断电本身不能推定门扇关闭，物理
  门位保持 `UNKNOWN`，不能据此释放整机占位。只有原清运员已在现场确认门扇关闭且锁
  输出为断电时，服务端才可释放整机占位，同时继续锁住原投口、原操作和新袋预留；
  其他正常投口此后可以工作。
...
- `cleanerCompletionConfirmed=true`、`cleanerPhysicalCloseConfirmed=true`、
  `cleanLockPowerState=DEENERGIZED`，以及最终稳定重量或明确终态称重故障必须同时形成
  完整结果，且香橙派已经把它写入本地可靠发件箱。称重故障时重量为 `null` 并携明确
  状态/故障码，不能用 0；完成事务把新基准置为无效并保持投口阻断。这里没有清运门物理
  检测，`cleanDoorPhysicalState` 始终为 `UNKNOWN`；任一响应、事件或审计都不得把人工
  确认描述为“门磁已确认关闭”。照片 URL 可以不完整，照片上传不阻断完成。
- `installedBagQr` 必须精确等于操作预留快照。P0 对不一致不猜测实际换袋，不创建记录、不更新袋或基准；消息进入技术隔离，原操作继续保持可核查状态。该防御不扩展为“袋码不一致处置业务”。
- 操作已经是 `PRE_UNLOCK_ENDED` 时到达的完成结果与“可信从未解锁”事实冲突，必须隔离并锁存设备一致性问题；不得回退终态或按结果强行换袋。操作缺少中间锁输出观察时，完整可信结果只有在自身能够证明首次解锁命令曾可能执行、清运员最终确认、当前锁断电，以及最终稳定重量或明确终态称重故障时才可越级收敛；不能补造物理开门/关门时间。
...
- P0 清运员小程序允许扫描一个本机构袋码后查看该袋的周期追溯；它不是可遍历的全机构袋目录，也不接受机构切换参数。每次请求都重新校验当前 `aud=miniapp` 清运员、租户、机构、小程序绑定和实时清运资格；其他机构袋统一按不存在处理。
...
1. 当前没有仍在驱动投递门、给清运电磁阀通电或执行其他物理动作的命令；全部相关
   投递门可靠关闭，清运电磁阀断电且旧命令不会再执行。清运门没有门磁，断电不能推定
   关闭，物理门位保持 `UNKNOWN`；仍处于 `RECOVERY_REQUIRED` 的清运操作必须由原
   清运员按 I-027 恢复，本接口不能替代其现场确认。因本故障而安全停住、仍保留占位/
   操作身份的原投递或清运作业允许存在，但本恢复事务既不结束它也不释放占位。
...
1. 当前 OneNet `openCleanDoor(cleanOrderId)`、`cleanGross`、`cleanTare` 和旧 D1 UART
   清运流程不能承载本章契约。正式协议必须以
   `operationUid/commandUid/eventUid/edgeEventSequence` 为根，能表达上下文可靠落盘、
   首次解锁可能执行、操作内再次解锁、锁断电、物理门位 `UNKNOWN`、清运员人工关门/
   完成确认、称重可靠性和业务确认。
