...
| 物模型功能点 | 类型 | 后端落点 | 状态 |
|--------------|------|---------|------|
| `openDeliveryDoor` | service | `DeliveryOrderService.openDoor`（**开启设备=建会话不建单**）→ `DeviceCommandService.sendOpenDoor(sn,doorIndex)` → `OneNetClient.openDeliveryDoor` | 真实下发已实现（门控 isConfigured），本地未测待凭证联调 |
