...
> **2026-09-16 现用生产 Docker 库已到 V84：**停止全部应用写入并确认活动投递、清运和软件
> 部署均为 0 后，从 V83 前向迁移至 V84。负责人明确要求本次不创建数据库备份，执行证据为
> `backup=SKIPPED_BY_OPERATOR`；因此失败处理只能前向修复，不能依赖本轮备份恢复。供应脚本
> 已同步到 V84，最终核对 138 张领域表、84 条成功迁移并重新锁定 schema owner；当前后端发布
> `20260916025440-863bcc8cca74` 已在 V84 健康运行。下文 V69/V72/V83 等数值均为对应历史阶段，
> 当前恢复续跑以脚本中的 V84 目标为准。
