...
> V66。2026-08-29 已另在全新腾讯云 CDB 正式空库完成 V1～V60、运行/备份账号和
...
6. 补齐 V39 当前小程序渠道、机构账号和永久设备归属触发器权限，再安装 V40～V66；这样较早纪元带业务数据续跑时，V48/V49 的数据回填不会先于触发器授权；
7. 撤销表名、列名重命名前遗留的触发器读取权限，再为 V63 新资产初始化和防降级触发器，以及 V64～V66 业务发布、单台验证和安全取消状态机授予精确权限；脚本会按系统权限表逐项核对最终权限并锁定 schema owner，已是 V66 的续跑环境也会幂等收敛；
8. 应用当前已冻结的表级/列级运行权限；
...
若现有数据库已完整到 V30～V66 中任一受支持纪元、owner 已锁定，使用：
...
123 张领域表，V64 为 129 张领域表，V65/V66 为 131 张领域表，并且迁移历史精确停在对应纪元。
低于 V66 时会用一次性新密码解锁 schema owner，只执行尚缺的前向迁移直到 V66，完成后立即
重新锁定；V66 不解锁 owner、不重复迁移。现场库已于 2026-09-06 从 V64 依次完成 V65、V66；
以后续跑时若已精确处于 V66，只做只读核对，不能假装重复迁移。
...
等持久化对象落到精确更新列。当前 grants 目录已按冻结状态机、不可变边界和 V66 DDL
补齐。
