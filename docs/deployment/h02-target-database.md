...
> V33；服务器目标空库已于 2026-08-04 重建到 V31，应用部署前需用续跑模式升级；
> 服务器既有执行结果另见
...
若现有数据库已完整到 V31、V32 或 V33、owner 已锁定，使用：
...
该模式要求 V31/V32 为 96 张领域表或 V33 为 97 张领域表，并且迁移历史精确停在
V31、V32 或 V33。V31/V32 会用一次性新密码解锁 schema owner，只执行尚缺的前向
迁移直到 V33，完成后立即重新锁定；V33 不解锁 owner、不重复迁移。
...
等持久化对象落到精确更新列。当前 grants 目录已按冻结状态机、不可变边界和 V33 DDL
补齐。
