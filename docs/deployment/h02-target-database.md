...
若现有数据库已完整到 V30～V55 中任一受支持纪元、owner 已锁定，使用：
...
V43/V44/V45 为 98 张领域表，V46～V51 为 99 张领域表，V52 为 112 张领域表，V53/V54 为 113 张领域表，V55 为 118 张领域表，并且迁移历史精确停在
对应纪元。低于 V55 时会用一次性新密码
解锁 schema owner，只执行尚缺的前向迁移直到 V55，完成后立即重新锁定；V55 不解锁
owner、不重复迁移。
...
等持久化对象落到精确更新列。当前 grants 目录已按冻结状态机、不可变边界和 V55 DDL
补齐。
...
- 对 74 张 P/O 表只授予矩阵明确列出的列级 `UPDATE`。
