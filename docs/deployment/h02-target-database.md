...
> V61。2026-08-29 已另在全新腾讯云 CDB 正式空库完成 V1～V60、运行/备份账号和
...
若现有数据库已完整到 V30～V61 中任一受支持纪元、owner 已锁定，使用：
...
113 张领域表，V55 为 118 张领域表，V56/V57/V58/V59/V60/V61 为 119 张领域表，并且迁移历史精确停在对应纪元。
低于 V61 时会用一次性新密码解锁 schema owner，只执行尚缺的前向迁移直到 V61，完成后立即
重新锁定；V61 不解锁 owner、不重复迁移。现有 V60 现场库属于受支持的续跑起点，不能直接激活要求 V61 的新后端。
...
等持久化对象落到精确更新列。当前 grants 目录已按冻结状态机、不可变边界和 V61 DDL
补齐。
...
- 对 75 张 P/O 表只授予矩阵明确列出的列级 `UPDATE`。
...
目录版本 31 包含 V52 注册续作、平台小程序会话、厂家袋扫码/更正、远程端口槽和维护会话，
...
`SEALED`（已封存）终态实际需要的投影列；V57 再增加微信授权展示包到期时间、验收/远程维护时钟质量和封存完成时钟质量的精确更新列，V58 不增加列或权限，只收紧恢复约束；V59 不增加列或权限，只扩大既有袋码数量检查约束；V60 增加验收证据与设备资产上的 MCU 远程升级线路能力三态列，并仅把资产投影列加入精确 UPDATE 白名单；V61 在 `ops_task_attempt` 增加可空的 `external_request_id`，并只把该结果字段加入既有表的精确 UPDATE 白名单。授权身份、验收证据、命令、可靠任务和创建事实
