...
若现有数据库已完整到 V30～V53 中任一受支持纪元、owner 已锁定，使用：
...
V43/V44/V45 为 98 张领域表，V46～V51 为 99 张领域表，V52 为 112 张领域表，V53 为 113 张领域表，并且迁移历史精确停在
对应纪元。低于 V53 时会用一次性新密码
解锁 schema owner，只执行尚缺的前向迁移直到 V53，完成后立即重新锁定；V53 不解锁
owner、不重复迁移。
...
等持久化对象落到精确更新列。当前 grants 目录已按冻结状态机、不可变边界和 V53 DDL
补齐。
...
目录版本 23 增加 V52 注册续作、平台小程序会话、厂家袋扫码/更正、远程端口槽和维护会话
状态收敛所需的精确列级更新；没有增加整表 UPDATE 或业务事实任意删除权限。厂家袋的袋码、
