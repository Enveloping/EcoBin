...
若现有数据库已完整到 V30～V56 中任一受支持纪元、owner 已锁定，使用：
...
113 张领域表，V55 为 118 张领域表，V56 为 119 张领域表，并且迁移历史精确停在对应纪元。
低于 V56 时会用一次性新密码解锁 schema owner，只执行尚缺的前向迁移直到 V56，完成后立即
重新锁定；V56 不解锁
owner、不重复迁移。
...
等持久化对象落到精确更新列。当前 grants 目录已按冻结状态机、不可变边界和 V56 DDL
补齐。
...
目录版本 29 包含 V52 注册续作、平台小程序会话、厂家袋扫码/更正、远程端口槽和维护会话，
...
`SEALED`（已封存）终态实际需要的 15 个投影列；授权身份、验收证据、命令、可靠任务和创建事实
