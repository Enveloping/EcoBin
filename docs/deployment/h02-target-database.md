...
> V62。2026-08-29 已另在全新腾讯云 CDB 正式空库完成 V1～V60、运行/备份账号和
...
若现有数据库已完整到 V30～V62 中任一受支持纪元、owner 已锁定，使用：
...
113 张领域表，V55 为 118 张领域表，V56/V57/V58/V59/V60/V61/V62 为 119 张领域表，并且迁移历史精确停在对应纪元。
低于 V62 时会用一次性新密码解锁 schema owner，只执行尚缺的前向迁移直到 V62，完成后立即
重新锁定；V62 不解锁 owner、不重复迁移。现有 V60 现场库属于受支持的续跑起点，必须依次完成 V61、V62，不能直接激活要求 V62 的新后端。
...
等持久化对象落到精确更新列。当前 grants 目录已按冻结状态机、不可变边界和 V62 DDL
补齐。
