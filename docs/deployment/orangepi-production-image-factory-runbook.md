...
> AA/DD、EE/EF或历史`factory_sim`作为通过依据。本手册的现行P7步骤已经改为原生身份、
> 配置、设备事实、一次START、完整结果保管和`RESULT_SAVED`确认；真实HMI、RS485、机构、
> 断电和ARM64首台试点仍未完成，不得据软件回归批准量产。现行证据链见
...
> policy（正式发布策略）仍为`UNLOCKED`；目标介质布局已为`LOCKED/QUALIFIED`，但仍须提供
> 匹配的资格证据。Air780E、板载 Wi-Fi AP、
...
必须同时看到 source、builder、apt、layout 均为 `LOCKED`。任何一项未锁定时，命令失败
是预期安全结果。以下行为一律禁止：
...
- MCU 已由线下烧录器预装当前批准的UART v2固件；香橙派不负责全新MCU首刷；
...
1. 系统检查：根分区扩容完成、磁盘可写且空间正常、machine-id 已首次生成、安全 GPIO 和
   出厂网络隔离健康；
2. MCU 身份：用UART v2 `QUERY_DEVICE_IDENTITY`取得当前启动编号、固件身份、最高命令序号和
   能力位。启动编号必须有效，能力位必须包含当前必要掩码`0x8100`，未知能力位、身份冲突或
   迟到的旧启动回复均失败；不能根据固件版本字符串猜测能力；
3. 原生配置和设备事实：按当前工厂配置完成分段配置应用，再用`QUERY_DEVICE_FACTS`核对相同
   配置版本及摘要、当前启动号、无活动作业、投递门最近有效控制为关闭、清运锁断电和称重
   事实。烟感、满溢距离/阈值及采集状态照实写入报告，缺测不能伪造成正常；
4. Bootloader线路：该项与业务UART协议分开。当前ECOBIN_UART固件不支持通过rc.26远程更新；
   没有另行批准且实装的BOOT0/NRST线路时必须报告`NOT_APPLICABLE`和
   `mcuRemoteUpdateCapable=false`，不得发送历史F2帧或据ROM探测宣称可远程升级；
5. 双摄：分别拍摄当前临时画面，再由操作员根据本次 nonce 确认 DECXIN 为箱外、icspring
   为箱内；不能沿用上一次启动或换线前的确认；
6. 称重：操作员在空载采集前填写本轮已知参考重量，单位为整数克；默认500g，也可填写
   400g、1000g或其他允许值。网页依次展示空载、加载、取下的样本、稳定值、差值、允许范围和
   判定。该输入只定义本轮验收比较标准并原样归档，不修改称重模块校准或生产计价参数；
7. 离线投递：动作前确认范围安全后只发送一次原生`START_DELIVERY_SESSION`。MCU自主完成首重、
   开门、串口屏按钮、关门和末重；验收器查询同一个作业的完整`WORK_RESULT`，可靠保存后发送
   身份绑定的`RESULT_SAVED`并取得回复。动作后再由操作员确认投递机构和周围区域安全；
8. 离线清运：动作前确认范围安全后只发送一次原生`START_CLEAN_OPERATION`。MCU自主完成清运
   屏幕、按钮、锁控制和前后称重；验收器保存同一作业完整结果并完成`RESULT_SAVED`确认，随后
   操作员独立确认清运门已经关闭；
9. 显示边界：完整页面、初始化、实时重量或二维码指令组原子进入UART3软件发送队列即视为
   已显示；队列不足必须零字节发布并重试。不等待USART发送完成、串口屏回执或屏幕读回；若
   现场未显示，按HMI工程、接线或屏幕硬件故障返工；
10. 安全交接：停止验收执行器并释放UART/GPIO/摄像头，重新取得同一MCU当前身份和设备事实，
   只在它们与P7报告一致时写入交接事实，随后首次启动编排才可进入联网阶段。

`factory_sim`、F3/F1、AA/DD和EE/EF只可用于历史兼容测试，不能生成rc.26的P7通过报告，
也不能作为真实MCU外设、屏幕或机构资格证据。

工单必须在开始前声明本轮结论类型。以下名称只用于工单和证据分类，不新增设备运行时
状态字段：

- `FLOW_VALIDATION`（接入流程正确性验证）：允许在独立测试环境使用符合相同协议和语义的
  模拟数据验证程序，但结果不能写成P7 `PASSED`，也不能进入注册、交接或封存；
- `PHYSICAL_HIL`（真实硬件资格验收）：必须连接真实 MCU 和本轮要签认的传感器、屏幕及
  执行机构，模拟数据不能满足对应物理资格项。

来源标记始终保留在证据中，但只在 `PHYSICAL_HIL` 或正式量产放行评审时影响物理资格结论。

原生START只要“可能已经写出”，程序就必须先可靠保存动作身份。浏览器刷新、手机断开或设备
断电都不能构造第二条START。重启后只查询原命令和原结果；已保存的完整结果继续完成
`RESULT_SAVED`，没有可靠结果时进入`RECOVERY_REQUIRED`，不以按钮、过程重量或动作片段重建
成功。现场安全无法重新确认时禁止后续动作、最终报告、交接、注册、封存和离厂。只有持久事实
明确证明原START零字节写出时，才允许在同一轮执行原动作。
...
本地报告为当前 image release、硬件配置摘要和 UART v2 MCU 身份/能力的有效 `PASSED`，投递
检查还必须明确保存动作后的 `operatorAreaSafeConfirmed=true`，并且当前没有恢复锁；只有这些事实同时成立，
首启编排器才允许切换防火墙并激活 Air780E RNDIS profile。验收以下事实：
...
- 手机不再看到该设备的出厂热点；
- UART5当前MCU身份/能力、原生设备事实、普通硬件、OneNet、COS和远程维护代理健康；
...
和日期。首台试点还必须附 HIL 抓包、串口/电平、本轮参考重量与全部采集样本、双摄角色、断电注入、
...
只有设备尚未注册、尚未封存，且不存在原生START“可能已写出”或其他恢复锁时，才可用上一
个已签名发布重新写卡。重写前仍需先验签，重写后重新完成全部验收。

### 15.2 禁止直接重写的情况

- 原生START可能已经写出：只查询原命令和原结果；完成相应投递区域安全或清运门关闭确认，
  保存当前轮次`FAILED`或恢复报告后再显式建立新一轮，不能构造或重发另一条START；
...
极性、改摄像头角色、篡改已经采集的本轮参考重量或样本、切换 PPP/QMI、开放热点转发或删除状态文件。
...
| MCU UART v2固件/能力位 | 身份与`0x8100`能力、配置/设备事实、一次START、结果保存确认、HMI与机构真机 |
| 摄像头型号或 USB 口 | 多次冷启动稳定路径、本次画面 nonce 和人工角色确认 |
| 称重模块或结构件 | 空载/本轮参考重量/取下的全部样本、差值与当前容差门槛 |
