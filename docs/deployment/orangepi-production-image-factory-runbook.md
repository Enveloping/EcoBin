...
7. 离线投递：确认门体安全后仅执行一次测试 BB/AA，观察屏幕、按钮、门锁/电机、红外和最终
   DD；DD 及复位后的原 F3 身份、F1 自检和串口静默全部通过后，恢复锁仍保持。操作员必须通过
   独立按钮重新确认投口驱动机构已停止、投口和周围区域安全；动作前的安全勾选不能复用，确认
   前禁止开始清运或最终封存；
...
网页必须显示并在报告中保存 `SIMULATED_PERIPHERALS`。操作员仍按上述顺序点击并完成动作
前/后安全确认；P7 通过后，首启编排器继续使用与真实 MCU 完全相同的联网、校时、注册、
OneNet 运行、P8 授权和封存路径，不能因来源标记增加、跳过或替换流程节点。

工单必须在开始前声明本轮结论类型。以下名称只用于工单和证据分类，不新增设备运行时
状态字段：

- `FLOW_VALIDATION`（接入流程正确性验证）：允许使用符合相同协议和语义的模拟数据；按实际
  状态节点判定流程是否通过，不把真实外设质量作为本轮阻断项，也不在每个后续节点重复提示
  模拟来源。
...
现场安全无法重新证明时保持 `RECOVERY_REQUIRED`，禁止清运、最终报告、封存、注册、正式业务
和离厂。不确定发送完成安全恢复及现场确认后，当前验收轮次生成 `FAILED` 报告并结束；只有操作员
显式开始新一轮后才能重新测试，同一轮绝不再次发送 AA、EE 或 F2。只有持久事实明确证明动作仍为
`ARMED` 且 `sendAttempts=0` 时，才允许同轮重试。
...
本地报告为当前 image release、硬件配置摘要和 MCU 身份的有效 `PASSED`，投递检查还必须明确
保存动作后的 `operatorAreaSafeConfirmed=true`，并且当前没有恢复锁；只有这些事实同时成立，
...
- AA、EE 或 F2 可能已经发送：先完成原动作的 MCU 复位、F3/F1 复验以及相应的投递区域安全或
  清运门关闭确认，保存当前轮次 `FAILED` 报告后再显式建立新一轮，不能在原轮次重发；
