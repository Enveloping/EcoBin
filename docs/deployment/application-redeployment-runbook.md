...
V55 增加 MCU 固件发布、人工灰度和进度事实；V56 增加封存授权与设备验收代次；V57 取消外部设备/微信时间与后端时间之间的数据库先后约束，增加时钟质量与微信授权展示包有效期；V58 补强封存空时钟质量、缺失微信创建时间及授权恢复约束；V59 将袋码批次和批内序号检查上限扩大到 500；V60 增加 MCU 远程升级线路能力三态事实；V61 为可靠任务尝试增加可空的外部技术请求号诊断字段；V62 为出厂进度查询增加可靠任务定位索引；V63 新增设备软件实际状态、永久管理层版本、管理架构代次和兼容性投影接收面，并把当前 epoch
门禁推进到 V63。执行前同样必须停止旧后端写入；迁移完成后只能激活理解 V63 的后端。V63 新增 4 张领域表和相应最小权限，但仍不包含业务程序更新下行、部署状态机或发布写入接口；从当前 V60 现场库升级时必须依次执行 V61、V62、V63 迁移并核对最小权限目录，再启动新 JAR。远程维护和 MCU 固件升级默认
