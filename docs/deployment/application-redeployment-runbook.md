...
门禁推进到 V66。执行前同样必须停止旧后端写入；迁移完成后只能激活理解 V66 的后端。当前现场 V64 库必须依次执行 V65、V66，核对 131 张领域表、66 条成功迁移及最终最小权限后，再启动新 JAR。业务程序远程下发仍由独立开关控制，数据库和页面就绪不等于已经允许设备更新。远程维护和 MCU 固件升级默认
