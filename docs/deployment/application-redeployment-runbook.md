...
V55 增加 MCU 固件发布、人工灰度和进度事实；V56 增加封存授权与设备验收代次；V57 取消外部设备/微信时间与后端时间之间的数据库先后约束，增加时钟质量与微信授权展示包有效期；V58 补强封存空时钟质量、缺失微信创建时间及授权恢复约束；V59 将袋码批次和批内序号检查上限扩大到 500；V60 增加 MCU 远程升级线路能力三态事实；V61 为可靠任务尝试增加可空的外部技术请求号诊断字段；V62 为出厂进度查询增加可靠任务定位索引，并把当前 epoch
门禁推进到 V62。执行前同样必须停止旧后端写入；迁移完成后只能激活理解 V62 的后端。V62 不新增表、列或权限，也不改变 OneNet 错误重试分类；升级时仍须先依次执行 V61、V62 迁移和最小权限目录核对，再启动新 JAR。远程维护和 MCU 固件升级默认
