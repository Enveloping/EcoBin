...
V55 增加 MCU 固件发布、人工灰度和进度事实；V56 增加封存授权与设备验收代次；V57 取消外部设备/微信时间与后端时间之间的数据库先后约束，增加时钟质量与微信授权展示包有效期；V58 补强封存空时钟质量、缺失微信创建时间及授权恢复约束；V59 将袋码批次和批内序号检查上限扩大到 500，并把当前 epoch
门禁推进到 V59。执行前同样必须停止旧后端写入；迁移完成后只能激活理解 V59 的后端。远程维护和 MCU 固件升级默认
