...
门禁推进到 V68。执行前同样必须停止旧后端写入；迁移完成后只能激活理解 V68 的后端。现场库
曾于 2026-09-06 依次执行 V65、V66、V67，并核对为 131 张领域表、67 条成功迁移及最终最小权限；
启用当前后端前还须前向执行 V68，并确认仍为 131 张领域表、68 条成功迁移。业务程序远程下发由独立开关控制，数据库和页面就绪不等于已经允许设备更新。远程维护和 MCU 固件升级默认
