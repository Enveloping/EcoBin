...
1. 确认目标 MySQL 已是完整 V48、99 张领域表、76 条权限定义；是否要求业务数据为空取决于当前部署是否已经承接试验数据，禁止为了满足旧手册而清空现有数据；
...
V48 一旦迁移完成，就不得切回不认识「独立设备安装资料」的旧应用继续写入；V47 对「技术中止的皮重测量代际」所建立的回退限制继续有效。
