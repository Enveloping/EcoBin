...
> 末次同步：2026-06-07
> 关联文档：权限与角色设计 [permission-design.md](../architecture/permission-design.md)、数据库设计 [database-design.md](../architecture/database-design.md)
...
> 详见 `docs/iot/onenet-thing-model.md` §8。
...
> 真实微信商家转账尚未接入，审核通过为占位逻辑（见 `docs/planning/open-items.md`）。
...
后端此刻**建单**：按 `device+deliveryToken` 幂等；取该设备「当前活跃用户」会话确定归属——命中则建单(`deliveryStatus=1`)、按 `投口单价 × 重量` 返现入余额；无活跃会话（过期/从未开启）则建无主单、不返现。详见 `docs/iot/onenet-thing-model.md` §8。
