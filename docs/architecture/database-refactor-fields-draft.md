...
建议暂沿用历史技术表名：`biz_clean_order`；其业务语义统一为清运记录。

清运记录不走审核。后台有权限人员可以直接修改允许修改的业务数据，保存后立即生效；目标模型保留设备原始重量/物理结果，并以独立只追加变更记录保存修改前后值、原因、操作者和时间。普通记录编辑不得顺带覆盖袋关系、重量基准、容量或设备安全状态。
...
| `audit_status` | 不进入目标模型 | 历史兼容列，现有值无业务含义 | 清运完成直接形成记录，后续直接修改也不经过审核；通过前向迁移删除 |
| `photo_open_outside` | `photo_open_outside` | 开门前箱外照片 URL | 沿用 |
| `photo_open_inside` | `photo_open_inside` | 开门前箱内照片 URL | 沿用 |
| `photo_close_outside` | `photo_close_outside` | 关门后箱外照片 URL | 沿用 |
| `photo_close_inside` | `photo_close_inside` | 关门后箱内照片 URL | 沿用 |
...
`new_bag_qr` 不进入新模型：新袋编号只更新 `biz_door_status.bag_qr`，不保存在清运记录。
...
> 第三轮修订：上面的“清运记录袋号只取数据库、不依赖设备上传”已被后续决定覆盖。
> 清运记录仍从清运开始时的状态快照取得旧袋号；设备完成事件上传的是换入的新袋号，用于更新投口当前袋，详见 §25.3。
...
清运配置只控制清运过程参数，不包含审核模式、免审条件或奖励规则。清运员完成确认直接形成记录；后台直接修改记录也不产生审核状态。
...
7. 清运记录在 v3 下是否仍同时保存 `gross_weight`、`tare_weight` 等重复重量快照？清运已经确认不审核且记录可直接修改；仍需保持设备原始快照与当前有效业务值分离。
8. 机构默认单价与投口个性化单价谁优先？
...
- `biz_clean_order.bag_qr` 固定表示本次清走的旧袋；换入的新袋只替换 `biz_door_status.bag_qr`，不写入清运记录。
...
1. 按 `(device_id, msg_id)` 幂等创建清运记录，记录的 `bag_qr` 写开始时捕获的旧袋号；
...
一个清运动作会接触旧袋和新袋，但清运记录按当时确认只记录一个袋号：
...
因此不再建议清运记录保存 `old_bag_qr + new_bag_qr`，当前已有的 `new_bag_qr` 应在正式迁移方案中退出。
为避免协议字段与记录字段同名却含义相反，完成事件建议把新袋字段明确命名为 `installedBagQr` 或 `newBagQr`，
不要笼统命名为 `bagQr`；清运记录的 `bag_qr` 永远表示被拿走的旧袋。

清运 v3 不预建清运记录，但仍建议生成 `clean_operation_id` 并持久化一条短生命周期清运操作上下文，
...
3. `biz_door_status.current_weight` 是传感器稳定原始毛重；皮重、净重也由设备上报。清运记录增加 `weight_before`、`weight_after`。
...
5. 是否接受新增短生命周期的 `clean_operation` 上下文表，用于在不预建清运记录的前提下可靠保存旧袋号和清运人？如果不新增，需要指定另一种可跨重启恢复的载体。
6. 清运记录是否长期同时保留 `weight_before + gross_weight`、`weight + net_weight` 两组同义列？当前稿按你的要求都记录，但推荐迁移完成后各保留一个权威字段，避免后续写出不一致值。
