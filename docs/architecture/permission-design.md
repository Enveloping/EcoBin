...
> 关联文档：[[database-design.md](database-design.md)]、未解决项见 [[open-items.md](../planning/open-items.md)]
...
- 当前实现只能通过小程序微信扫码自动注册，**不允许后台手动创建**。该入口限制已被 2026-07-22 新需求基线取代：目标系统允许直接进入小程序或扫描设备二维码注册；扫码只额外形成可空的地推来源归因。
