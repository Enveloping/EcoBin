...
精确重建到 V31（96 张领域表、77 条权限定义、其余业务数据 0）；当前代码已增加 V32～V52，
部署含当前纪元门禁的应用前必须现场核对服务器实际版本，并用 H-02 续跑模式完成前向升级到 V52（112 张领域表、76 条有效权限定义）；
...
- V1～V31 已完成前向迁移和 MySQL 权威验证；V32～V51 保留既有身份、设备、可靠任务和资金状态演进；V52 增加设备自注册、OneNet 供应、厂家验收小程序、初始袋标签占用和按需远程维护；F-07 又完成固定 V1 marker，当前代码已推进为
  V52 epoch/readiness guard、最小 `ecobin_app` 空业务库启动和 Fake 外联硬阻断；
