...
> [!IMPORTANT]
> 2026-09-04，第六阶段“本地验证设备更新器”已形成默认关闭的仓库候选：业务发布包精确排除
...
> 桥接服务；业务更新与 MCU 更新不能并发。Windows 全量为 `1839 passed, 104 skipped,
> 5 subtests passed`，Linux/Python 3.11 权限/助手/业务包/协调器专项为 `72 passed`，镜像工具专项为
> `38 passed, 3 skipped, 24 subtests passed`。本轮未生成或修改镜像、正式签名密钥及现场包，
> 也未向 TF 卡或香橙派写入；现有 v21 HIL 镜像 SHA-256 仍为
> `d0d7199b23433b9929363cd2ba0be5a4fbaddc57d2fa54a7541ca49818946bab`。下次写卡窗口须构建新
> 镜像，并完成首次成功迁移、第二版本故意失败回滚和关键阶段断电恢复，才能关闭第六阶段。
...
精确重建到 V31（96 张领域表、77 条权限定义、其余业务数据 0）；当前代码已增加 V32～V63，
部署含当前纪元门禁的应用前必须现场核对服务器实际版本，并用 H-02 续跑模式完成前向升级到 V63（123 张领域表、76 条有效权限定义）；
