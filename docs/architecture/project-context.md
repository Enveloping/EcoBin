...
> 数据。当前测试后端仍连接原单机 MySQL 的 V59 数据库并继续承接写入，新 CDB 尚未切换，
...
> 项目负责人进一步明确：当前后端连接的 V59 数据库和现用 COS 都是测试环境，尚未完成的
...
> 目标产品，仓库文档落后；无需重复导入。指定设备最新证据仍为 `tenant_id=NULL`、
> `organization_id=NULL`，机器验收 `PASSED` 且当前代次已 `SEALED`。因此当前测试流程的
> 下一步是平台永久分配测试租户，再由该租户永久分配测试机构；机构分配事务随后自动建立
> 投口、厂家初始袋占用和配置 v1，并在设备 `APPLIED` 后自动发起初始空袋皮重。
...
精确重建到 V31（96 张领域表、77 条权限定义、其余业务数据 0）；当前代码已增加 V32～V60，
部署含当前纪元门禁的应用前必须现场核对服务器实际版本，并用 H-02 续跑模式完成前向升级到 V60（119 张领域表、76 条有效权限定义）；
