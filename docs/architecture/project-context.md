...
> 当前推进：[MCU与香橙派精简实施计划](../planning/mcu-edge-simplified-implementation-plan-2026-09-13.md)。[S4发布清单与隔离包保管](../../hardware/docs/review/simplified-release-custody-s4-2026-09-14.md)已将业务包对齐为63文件/schema40/摘要a376d660…，完整运行包74文件；后台验证器同步。旧动态动作、过程回执及恢复关门共享夹具均已迁移为现行自主业务或冻结历史只读验证，硬件全量回归4185通过/123环境跳过/5子测试通过。UART rc.24二维码URL链路已有本地跨端候选；真实串口屏/RS485/换袋/断电HIL、Linux/ARM64安装、热点/固件管理和部署仍未完，仍不是可部署版本。数据库候选V82、OneNet2.4.0/UART rc.24，完整MCU Flash59676/RAM12688。下方旧候选为历史。

> 2026-09-14二维码URL收口：[UART v2设备二维码URL可靠应用计划与实现记录](../../hardware/docs/review/uart2-device-entry-url-plan-2026-09-14.md)。项目负责人确认信任MCU写出完整串口屏指令即显示，否则无法保证任何串口屏显示；具体实现把完整`page0.qr0.txt`命令原子写入UART3软件发送环作为唯一成功事实，不追踪USART完成或HMI回执。rc.24已实现BEGIN/PART/COMMIT/APPLY_RESULT、Pi SQLite重启/重连重发、可靠OneNet应用结果、后台任务完成证明和验收证据版本5；旧`Stored`字段仍只证明Pi保存。25项契约检查和Keil完整重建已通过，但尚未部署、烧录或用真实串口屏验证。
