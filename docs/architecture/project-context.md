...
> 2026-09-16 v44本地候选：超声波测距按业务无关的辅助设备事实处理，投递/清运不再启动、
> 保留或等待业务内测距组；验收直接读取最近MCU事实快照，不再因测距观测较旧、不可用或
> 尚未观测而阻止。关门行程等待为3秒，继续/结束
> 选择窗口仍为30秒；触摸屏在关门后称重完成前快速发出的END，只在本轮关门已接受后暂存，
> 取得本轮末重后精确绑定并立即结束，不能跨START或轮次。UART推进rc.27但线格式不变，
> OneNet推进2.5.0，数据库候选V84；后台验收失败结论可返回v44热点页，避免无授权时永久显示
> 等待。源码提交为`25fd2d45`；最终工厂验收镜像2,571,108,352字节，SHA-256为
> `503913146c5e76aca09c6dd9e91d7b1906487af5c488c1a3e74686ee095f79b4`，文件树差异审计及断网
> 只读ARM64冒烟通过。尚未部署后台、写卡、烧录MCU或完成真实HIL。见
> [v44记录](../../hardware/docs/review/v44-acceptance-distance-and-delivery-timing-2026-09-16.md)。
