...
> 的事实入库。激活当前代码前必须先把现场库从 V67 前向迁移到 V68。OneNet 当前
> 候选为 17 个同步服务 / 21 个事件 / 0 个属性。旧直连设备继续沿用原业务规则；已经切换到
> 永久管理层的设备只有在实际安装事实明确兼容且未更新时才可开始新投递或清运。Web 展示中文
> 结论，不把内部状态码作为主文案。项目负责人已在 OneNet 控制台完成当前物模型导入。
...
- 当前物模型机器来源是 `contracts/onenet/thing-model.mapping.yaml`，控制台导入件是
  `contracts/onenet/generated/onenet-thing-model.candidate.json`，现为 17 服务 / 21 事件 / 0
...
- 当前 OneNet 候选为 17 服务 / 21 事件；其中既有 15 服务 / 18 事件的逐项处理结论见
  `hardware/docs/review/onenet-edge-handling-decisions.md`。V63 增加设备软件实际状态事件，V65/V66
  又增加业务更新开始/取消服务及进度/取消结果事件；后续讨论结果统一增量写入该文档。
- 当前待冷启动镜像是从干净提交 `756ff54e78f1b28270f75eb5f0f17fc21481bf6d` 构建的 v31，
  不是首次真实冷启动暴露缺陷的原始 v30。v31 软件、整镜像和永久层离线检查、指定 TF 卡写入及
  写后全范围回读已经通过，但冷启动和真实远程更新尚未完成；在这些现场事实取得前，不能把后端已经允许
  识别该运行时理解为设备已完成远程更新验收或具备量产资格。
...
精确重建到 V31（96 张领域表、77 条权限定义、其余业务数据 0）；当前代码已增加 V32～V68，
现场现用库已经用 H-02 续跑模式前向升级到 V67（131 张领域表、76 条有效权限定义）；以后新增
...
systemd 边界专项为 `258 passed`；生成/跨语言契约校验为 `24 passed, 0 notes`，覆盖 98 个
生成文件、17 条命令、21 类事件和 38 份线级样例。香橙派当时的默认路由/DNS 波动按
