...
> 修复，因此 v16 只能作为历史离线候选。新的 v17 已从干净提交 `16cd154f` 完整重建，无秘密
> 候选、受控 HIL 副本、注入后审计、第二次独立复验和最终只读身份检查均通过；它是下一次
> 写卡源，但仍须完成全范围回读和断电冷启动。在低权限全业务回归、MCU 更新迁移、断电恢复
> 及真实动作防重验收完成前，第四阶段不得启用。v17 证据见
...
> v17 离线复验通过后，服务器实际机器验收允许列表已追加
> `hardware-runtime-20260903-17`。后续复核又移除了从未写卡的 v15，以及缺少最终权限修复、已
> 降为历史候选的 v16；当前 v13 继续保留。两次修改均保留 root 私有备份，最终生产预检、应用
> 重载、后端与 Web 健康检查、容器内精确核对和运行秘密隔离探针均通过。这只表示后端能够认可
> 未来的 v17 上报，不表示 v17 已经写卡或产生真实设备验收证据。脱敏部署证据见
...
> 均未接入或启用。这些新增代码是在 v17 构建之后完成，不能把 v17 离线证据当作本轮镜像证据。
