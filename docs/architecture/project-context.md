...
> [!IMPORTANT]
> 2026-08-07 项目负责人重新冻结 D-047 / I-057 / V36：设备资产只允许一次性永久分配租户和机构；部署实例、部署码、回收/调拨、现场人工验收和人工经营开关全部退出。平台在分配前自动机器验收，机构只安装联网；小程序使用永久设备公开码，OneNet/COS/边缘 v2 不含部署码。禁用/报废只阻止新业务，既有作业继续收敛，物理安全事件例外。当前无生产数据，实施不保留旧协议兼容层。完整裁决见 [`permanent-device-ownership-v36.md`](permanent-device-ownership-v36.md)。
...
精确重建到 V31（96 张领域表、77 条权限定义、其余业务数据 0）；当前代码已增加 V32～V36，
部署含当前纪元门禁的应用前必须现场核对服务器实际版本，并用 H-02 续跑模式完成前向升级到 V36（93 张领域表、76 条有效权限定义）；
...
- V1～V31 已完成前向迁移和 MySQL 权威验证；V19 已增加投递配置管理，V20 已删除清运审核列/约束并以清运修改留痕表替换历史审核修订表，V21 已补齐无门磁正常清运事实，V22～V31 又补齐设备运行与资金恢复事实；V32 将机构 AppSecret 改为数据库明文字段并废弃历史假引用，V33 增加运营治理写操作的并发幂等事实表，V34 增加平台按机构用户筛选审计日志所需的时间索引，V35 增加免确认收款授权、授权观察和新旧提现收款模式快照，V36 删除设备分配/部署/回收兼容模型并把所有保留事实改归永久资产；F-07 又完成固定 V1 marker，当前代码已推进为
  V36 epoch/readiness guard、最小 `ecobin_app` 空业务库启动和 Fake 外联硬阻断；V36 已在一次性 MySQL 8.4.10 上通过 V1→V36 与只读启动验证，
