...
精确重建到 V31（96 张领域表、77 条权限定义、其余业务数据 0）；
...
- V1～V31 已完成前向迁移和 MySQL 权威验证；V19 已增加投递配置管理，V20 已删除清运审核列/约束并以清运修改留痕表替换历史审核修订表，V21 已补齐无门磁正常清运事实，V22～V31 又补齐设备运行与资金恢复事实；F-07 又完成固定 V1 marker，当前已推进为
  V31 epoch/readiness guard、最小 `ecobin_app` 空业务库启动和 Fake 外联硬阻断，
