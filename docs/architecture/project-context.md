...
V-02 的软件前置全部解除，转为 `ready`，但尚未获得实施授权。当前共 `done` 12、
`ready` 1、`in-progress` 1、`blocked` 15。其他任务没有因前置推进而自动获得实施授权。
