...
> 2026-09-14 异常投递人工结束并报废：[实现与验证](../planning/abnormal-delivery-retirement-2026-09-14.md)已完成本地候选。平台管理员只能在投递结果未知、无订单与物理结果、无活动任务租约且设备连续离线至少10分钟时，经现场断电和机构安全确认，将原会话、精确占用、资产报废及剩余任务取消作为一个事务处理；不产生订单或资金效果。现网只读核对了旧设备阻塞形态，未修改现网数据；当前未提交、未部署，且依赖V80字段，现网V72不能单独热替换应用。
