...
- 2026-08-03 最终把“能否新建物理业务”收敛为 OneNet 传输在线事实：资产级 `oneNetConnectionStatus` 的唯一权威来源是北向 `deviceOnline` / `deviceOffline` 生命周期通知；只有明确 `ONLINE` 才能创建新的投递或清运任务，`OFFLINE` 和尚无生命周期事实的 `UNKNOWN` 都阻止创建。普通已鉴权消息、运行快照、OneNet 下行错误和数据库时间差都不能覆盖或推断该在线状态。接口仍可返回部署级 `edgeConnectionStatus` 供诊断，但它不再是后端创建投递/清运的准入条件。
- 后端只判断自己能够权威确认的条件，例如用户和机构权限、设备归属/验收/经营开关、整机是否已有后端作业占用、机构规则、OneNet 当前是否明确在线，以及已经由可信重启事件形成的清运中断锁。称重、烟感、投口执行器、满溢和香橙派本地清运锁等需要观察现场的条件，由香橙派收到命令后在物理动作开始前判断；拒绝时上报稳定错误码，后端不再用较早的运行快照替香橙派猜测当前物理状态。
...
- 固定帧只有单投口且无 ACK、CRC、流程编号和状态查询。香橙派不重发物理启动命令，
  ...
  SQLite/MCU 作业冲突锁或部署身份启动锁，也不要求把 systemd 的 `Restart=on-failure`
  改为 `Restart=no`。
...
精确重建到 V31（96 张领域表、77 条权限定义、其余业务数据 0）；当前代码已增加 V32～V34，
部署应用前必须用 H-02 续跑模式完成前向升级到 V34（97 张领域表）；
...
- V1～V31 已完成前向迁移和 MySQL 权威验证；V19 已增加投递配置管理，V20 已删除清运审核列/约束并以清运修改留痕表替换历史审核修订表，V21 已补齐无门磁正常清运事实，V22～V31 又补齐设备运行与资金恢复事实；V32 将机构 AppSecret 改为数据库明文字段并废弃历史假引用，V33 增加运营治理写操作的并发幂等事实表，V34 增加平台按机构用户筛选审计日志所需的时间索引，三版均尚需在服务器执行；F-07 又完成固定 V1 marker，当前代码已推进为
  V34 epoch/readiness guard、最小 `ecobin_app` 空业务库启动和 Fake 外联硬阻断，
