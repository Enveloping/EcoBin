...
> HIL 镜像和永久管理层离线检查已通过，生产认可列表也已由 v30 精确替换为 v31；但 v31 尚未
> 写卡，真实远程更新也未执行，下发开关继续为 `false`。证据和现场顺序见
...
- 当前待写卡候选是从干净提交 `756ff54e78f1b28270f75eb5f0f17fc21481bf6d` 构建的 v31，
  不是首次真实冷启动暴露缺陷的原始 v30。v31 软件、整镜像和永久层离线检查已经通过，但写卡、
  写后全范围回读、冷启动和真实远程更新尚未完成；在这些现场事实取得前，不能把后端已经允许
