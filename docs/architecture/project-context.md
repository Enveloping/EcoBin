...
精确重建到 V31（96 张领域表、77 条权限定义、其余业务数据 0）；当前代码已增加 V32～V33，
部署应用前必须用 H-02 续跑模式完成前向升级到 V33（97 张领域表）；
...
- V1～V31 已完成前向迁移和 MySQL 权威验证；V19 已增加投递配置管理，V20 已删除清运审核列/约束并以清运修改留痕表替换历史审核修订表，V21 已补齐无门磁正常清运事实，V22～V31 又补齐设备运行与资金恢复事实；V32 将机构 AppSecret 改为数据库明文字段并废弃历史假引用，V33 增加运营治理写操作的并发幂等事实表，两版均尚需在服务器执行；F-07 又完成固定 V1 marker，当前代码已推进为
  V33 epoch/readiness guard、最小 `ecobin_app` 空业务库启动和 Fake 外联硬阻断，
