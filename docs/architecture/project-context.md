...
> 当前推进：[MCU与香橙派精简实施计划](../planning/mcu-edge-simplified-implementation-plan-2026-09-13.md)。[S4发布清单与隔离包保管](../../hardware/docs/review/simplified-release-custody-s4-2026-09-14.md)已将业务包对齐为62文件/schema40/摘要f45f4370…，完整运行包73文件；后台验证器同步。五批现行业务/历史兼容测试已迁移：前两批扩大合并437项，第三批200项、第四批261项；第五批把101项旧动态动作测试换为34项真实v1只读兼容，并增加18项准确标注局限的恢复永久账本输入。业务采样与可选满溢诊断已分离，现行恢复测试夹具依赖环已拆。余下约202项恢复候选共享测试、二维码URL新链路、真实RS485/换袋/断电HIL、Linux/ARM64安装、热点/固件管理和部署仍未完，仍不是可部署版本。清运中断人工袋确认候选保持V81/138张领域表/运行权限目录42；OneNet2.3.0/UART rc.23，完整MCU ROM56436/RAM12008。下方旧候选为历史。

> 2026-09-14新增发布缺口：[UART v2设备二维码URL可靠应用计划](../../hardware/docs/review/uart2-device-entry-url-plan-2026-09-14.md)。当前后台命令只能证明Pi保存，不能证明MCU/HMI应用；已确认以MCU把完整指令原子交给UART3发送队列作为屏幕显示成功，不要求屏幕回执。因242字节载荷装不下192字节URL、命令身份和摘要，计划采用BEGIN/PART/COMMIT/APPLY_RESULT分片事务；URL变化、MCU重启或UART重连重发，业务中延后。协议生命周期分组、后台完成语义、验收新证据、ROM/RAM和真实屏幕验证尚未实施，rc.23状态不因此改变。
...
> 已确认非重启的最终称重失败保留系统异常订单及人工审核，与重启丢数据仅作问题记录
> 区分；故障未恢复不接新投递，恢复且其他条件满足后自动接单。准确度问题由人工发现
...
- 当前 OneNet 候选为 18 服务 / 22 事件；其中既有 15 服务 / 18 事件的逐项处理结论见
