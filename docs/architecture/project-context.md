...
> [!IMPORTANT]
> 2026-07-27 已完成需求、P0 范围、业务模型、系统架构、数据库设计、接口设计和详细设计修订；投递改为一次 session 一单/设备本地继续，清运改为电磁阀解锁/人工关门确认。DD-004 与修订后的 PDD-001 已写回基线，29 项正式 Markdown 任务也已同步。H-01、H-02、F-01、F-02、F-03、F-04、F-05、F-06、F-07、F-08、F-09、F-10、V-01 已获授权并完成；F-08 的 Fake 可靠任务 tracer、MySQL 8.4 验收和两项 P1 复审已收口，V-01 的六项 P1 复审修复、真实 MySQL 八项专项和全仓回归也已收口。F-11 已获授权并完成固定帧 MCU 适配、可靠照片上传和配置命令软件纵切，继续处于 `in-progress`；H-02 已通过本地 MySQL 8.4.10 开发演练及服务器整改阶段 0～3 验收并转为 `done`。V-02 software 已完成 MySQL 8.4、全仓和客户端自动验收，任务级仍等待真实微信 integration/acceptance。其他任务仍须逐项授权，`ready` 只表示依赖允许领取。正式上游依次为
...
- 物理 UART 通常至少连接交叉的 TX/RX 和共地 GND。F-11 已把 `hardware/main.py`
  的正式入口切向 SQLite 新骨架；规范 `uart-v1` 实现仍可显式选择，当前现有单片机
  使用双方确定的 `AA/BB/EE` 下行与 `DD/EF` 上行固定帧适配器；
  新入口已修复协议方向、同帧原字节重试、ACK/NACK 匹配、MCU 事件先持久化后 ACK 和
  `QUERY_STATE` 缺段安全锁，并完成 `APPLY_CONFIGURATION` 分段配置软件纵切。
- 最近一次 UART 审计见 `hardware/docs/review/uart-protocol-audit.md`，当前适配实施记录位于
  F-11 独立 worktree。F-10 的 OneNet/规范 UART 机器来源与通用三语言黄金样本已完成；
  固定帧只有单投口、无 ACK/CRC/流程编号/状态查询等限制，缺失事实必须明确拒绝或保持
  `UNKNOWN/NOT_SAMPLED`，不能伪造为 Registry 能力。
...
V-02 software 已完成，任务级因真实微信 integration/acceptance 尚未开始而保持
`in-progress`。当前共 `done` 13、`ready` 0、`in-progress` 2、`blocked` 14。
...
- V-01 身份/Web 纵切已完成复审并获准合入；V-02 software 已在独立 worktree 中
  完成并通过自动验收，真实微信联调和人工验收仍受阶段门槛约束；
- F-11 已合入固定帧适配和可靠照片上传，下一步补齐其他业务命令状态机及强杀/断网
  故障注入；H-03 在同一适配版本上执行固定帧线路、屏幕状态机和物理行为真机验收；
...
当前 OneNet MQTT 已联通，COS 仍待完整软件闭环，固定帧 MCU 适配仍在独立 worktree；
