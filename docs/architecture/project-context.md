...
> 当前推进：[MCU与香橙派精简实施计划](../planning/mcu-edge-simplified-implementation-plan-2026-09-13.md)。检查点3178a994之后的[原生业务入口/配置/正常收尾](../../hardware/docs/review/native-business-entry-s1-2026-09-13.md)已接实际main分支，正常业务无需逐过程/动作确认。永久账本请求已改为后台等待，串口主循环继续运行；本次提交前统一129项相关回归通过，111份生成物一致。此前276项入口、83项Java、15项MySQL配置及25项契约验证见记录（集合重叠）。问题归档收尾仅完成本地组件、尚未接Runtime；故障出口、S3断网释放、热点/固件管理/现场验证未完，不是可部署版本。OneNet2.3.0/UART rc.23，完整MCU ROM54600/RAM11824。下方旧候选为历史，以新计划最新执行记录为准。
