...
> 2026-09-16 设备管理界面调整已部署。设备列表减少默认字段、缩短设备编号、固定多投口内容
> 高度并补齐表格外侧边界；设备详情重新按使用顺序组织信息，隐藏低频技术信息。列表后端按
> 当前袋和最新设备事实返回整机满溢结论。生产发布为
> `20260916090730-99d26dc4338b`（提交 `99d26dc4`），数据库未迁移，仍为 V84/138 表；后端和
> Web 均健康且重启数为 0。见
> [部署记录](../operations/device-web-refinement-deployment-2026-09-16.md)。

> 2026-09-16 v46 已完成镜像写卡、后端发布和后台认可。设备运行时
> `hardware-runtime-20260916-46` 来自提交 `d76b16ab`；最终工厂镜像 SHA-256 为
> `56d66f29b4b2b616a62d2e4b29f0f527745ec63b1f79009da2f5eb77b1b342e1`，指定 Windows 磁盘 1
> 的完整写入、刷新和全镜像范围回读摘要一致。后端/API 提交 `d9afcd85` 固定原生配置档位并
> 补充设备详情事实，`eff9af5e` 同步新业务模块白名单；生产发布
> `20260916083810-eff9af5ea9df` 已激活，数据库仍为 V84/138 表。后台只追加 v46 允许项，
> 后端/Web 健康、重启数为 0，远程业务程序下发仍关闭。尚未装卡冷启动或完成真实 HIL，见
> [v46 写卡与后端发布记录](../operations/orangepi-v46-write-backend-deployment-2026-09-16.md)。

> 2026-09-16 v45本地候选：超声波测距按业务无关的辅助设备事实处理，投递/清运不再启动、
> 保留或等待业务内测距组；验收直接读取最近MCU事实快照，不再因测距观测较旧、不可用或
> 尚未观测而阻止，也不再用历史满溢投影阻止投递。关门行程等待为3秒，继续/结束
> 选择窗口仍为30秒；触摸屏在关门后称重完成前快速发出的END，只在本轮关门已接受后暂存，
> 取得本轮末重后精确绑定并立即结束，不能跨START或轮次。UART推进rc.27但线格式不变，
> OneNet推进2.5.0，数据库候选V84；后台验收失败结论可返回v45热点页，避免无授权时永久显示
> 等待。源码提交为`1551c5be`；最终工厂验收镜像2,571,108,352字节，SHA-256为
> `cfdb73990bf4d1433f6892d503996f897360f24dcd1106b13e7d698f3c33c9a1`，文件树差异审计及断网
> 只读ARM64冒烟通过。经负责人明确确认覆盖磁盘1后，指定TF卡的完整写入、刷新和全镜像
> 回读摘要一致；生产后台也已只追加`hardware-runtime-20260916-45`，独立复核健康且远程业务
> 程序下发仍关闭。尚未烧录MCU、装卡冷启动或完成真实HIL。见
> [v45记录](../../hardware/docs/review/v45-distance-business-decoupling-2026-09-16.md)及
> [写卡与后台认可记录](../operations/orangepi-v45-write-and-backend-allowlist-2026-09-16.md)。v44因
> 残留历史满溢准入而被取代，不应再写卡。

> 2026-09-16 后端已部署验收 v5 与运行重量 UUIDv5 兼容，生产库从 V72 前向迁移至 V83，
> 当前发布为 `20260915160954-736a9e1adeef`。后端只为运行重量测量编号放宽到 RFC
> UUIDv4/UUIDv5，其他编号不放宽；未来设备源码已改为持久化随机 UUIDv4，但已写卡 v43 保持
> 不变。激活后至少 22 条验收证据和 231 条运行快照均进入可靠处理，两类旧格式隔离
> 新增为 0；旧挑战只作无动作处理。当前新挑战随后已可靠下发并收到匹配证据，后端判定、平台
> 确认和设备确认回执均闭环；真实结果为 `FAILED / SENSOR_SELF_TEST_FAILED`，因此格式链路已经
> 恢复，但整机验收仍未通过。见
> [v43 镜像与后端部署记录](../operations/orangepi-v43-backend-allowlist-deployment-2026-09-15.md)。

> 2026-09-15 UART通信故障候选已改为
> [按请求计时与精确回复自动恢复](../../hardware/docs/review/native-control-communication-failure-s2-2026-09-13.md)：
> 只有需要回复的请求完整写出后连续5秒没有匹配回复才超时；空闲静默、数据过旧和香橙派线程
> 卡顿不再制造MCU通信故障。迟到或故障后新请求的精确回复只恢复通信，不恢复原失败业务或
> 业务价值。设备验收的摄像头/COS、远程维护socket和问题证据生成已移出UART所有者线程。
> Python 3.11完整硬件回归为4334通过、126环境跳过、1失败、5子测试通过；唯一失败是既有
> Windows SQLite强杀恢复竞态，随后单项原样重跑通过。源码已提交为`244dd4ea`并进入
> `hardware-runtime-20260915-43`；两次候选构建逐字节一致，最终受控镜像SHA-256为
> `6c9a97e21dec1be8c6a5d2a96b735ab401cf44eb8393c008d3e64ef129325e2f`，差异审计和断网只读
> ARM64冒烟通过。经负责人明确确认覆盖磁盘1后，已向序列号`121220160204`的USB TF卡完整
> 写入并刷新；写入和全镜像范围回读均为2,571,108,352字节，回读摘要与源镜像一致。生产后台
> 已只追加v43并独立复核；当前卡已是v43但尚未装卡冷启动，真实HMI、RS485、机构、投递/清运
> 及断电HIL仍未执行。见
> [v43镜像与后台认可记录](../operations/orangepi-v43-backend-allowlist-deployment-2026-09-15.md)。
