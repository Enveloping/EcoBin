...
> 结论，不把内部状态码作为主文案。OneNet 控制台导入仍需以实际导入结果单独确认。
...
> [!IMPORTANT]
> 2026-09-06，第八阶段真机候选已经固化为 v30：镜像版本
> `0.1.0-single-card.20260906.30`，硬件运行时 `hardware-runtime-20260906-30`，HIL 镜像
> SHA-256 为 `53facbdfe7c8ae62b5ac304193e8f124ff95d6241bbe26952dbd0437710a0576`，大小
> 2,571,108,352 字节。它从永久 systemd 服务层显式开启“通信代理只路由业务更新命令”和
> “更新器允许远程业务更新”，并只给更新器访问固定北京私有 COS 域名所需的 DNS/HTTPS
> 网络权限；v29 因缺少这两项运行权限而不得写卡。v30 已通过 Linux/Python 3.11 完整套件、
> 远程更新专项、两轮整镜像审计和永久层专项审计，生产认可列表也已由 v29 精确替换为 v30；
> 但 v30 尚未写入 TF 卡，真实远程更新也未执行，下发开关继续为 `false`。证据和现场顺序见
> [`v30 单卡远程业务更新镜像证据`](../../hardware/image-artifacts/evidence/hil-remote-business-update-20260906-30/README.md)
> 与
> [`v30 远程业务更新验收手册`](../operations/orangepi-business-runtime-remote-update-readiness-2026-09-06.md)。
...
- 当前待写卡候选是从干净提交 `61d2620b736b1c36ffcaddec2216207652963775` 构建的 v30，
  不是缺少远程启用参数的 v29。v30 软件、整镜像和永久层离线检查已经通过，但写卡、写后
  全范围回读、冷启动和真实远程更新尚未完成；在这些现场事实取得前，不能把后端已经允许
