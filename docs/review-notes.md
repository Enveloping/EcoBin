...
#### 1.【中·设计缺口】终端用户登录后无业务接口可用
`SecurityConfig` 中 `/api/business/delivery/**` 限定为 `SUPER_ADMIN, TENANT`，而 `wxLogin` 注册的小程序用户是 `role=1`（USER）。
普通用户登录后访问投递订单等接口会被 403。`/api/system/user/**`、`/api/statistics/**` 同样不含终端角色。
- **待确认**：这是「C 端接口尚未开发」还是漏配。
- 若后续开发 C 端接口，需为终端角色（1/2/3）单独规划路径与权限。

#### 2.【中·运维隐患】TokenInvalidationRegistry 重启即失忆
...
#### 4.【中·需确认】updateById 置 null 依赖默认字段策略
