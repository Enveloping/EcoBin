...
fail() {
    printf 'business-builder-launch=FAIL: %s\n' "$1" >&2
    exit 1
}
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --version-name)
            ...
            ;;
        --release-sequence)
            [[ $# -ge 2 ]] || fail "--release-sequence requires a value"
            release_sequence="$2"
            shift 2
            ;;
        --signing-private-key)
...
for value in "${output_directory}" "${release_id}" "${version_name}" \
    "${release_sequence}" "${signing_private_key}"; do
    [[ -n "${value}" ]] || fail "all business release inputs are required"
done
...
[[ -z "${repository_status}" ]] \
    || fail "formal business releases require a clean repository"
...
[[ ! -e "${output_parent}/${output_name}" ]] \
    || fail "business output directory already exists"
...
docker run --rm --platform "${builder_platform}" \
    ...
    --mount "type=bind,src=${signing_private_key},dst=/business-input/signing-private.pem,readonly" \
    --mount "type=bind,src=${output_parent},dst=/output" \
    "${builder_reference}" \
    bash /workspace/tools/orangepi-image/bootstrap-builder.sh --business-release-only \
    --release-id "${release_id}" \
    --version-name "${version_name}" \
    --release-sequence "${release_sequence}" \
    --source-root /workspace/hardware \
    --output-directory "/output/${output_name}" \
    --signing-private-key /business-input/signing-private.pem
