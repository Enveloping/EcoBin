...
"@)
        ...
        $resumeLayoutValid = (
            ...
            ($existingDomainTableCount -eq 99 -and
                ...
                $existingMaxVersion -eq 49)
        )
        if (-not $resumeLayoutValid) {
            throw (
                "Migrated resume requires a complete V30 through V49 " +
                "target database"
            )
        }
        if ($existingMaxVersion -lt 49) {
...
"@ | Out-Null
        ...
        Invoke-FlywayMigration -Target 49 -OwnerPassword $ownerPassword
        $migrationCompleted = $true
...
"@
    ...
    $historyCount = [int](Invoke-RootSql `
        -Database $DatabaseName `
        -Sql "SELECT COUNT(*) FROM flyway_schema_history WHERE success = 1;")
    if ($historyCount -ne 49) {
        throw "Expected forty-nine successful Flyway migrations"
    }
...
"@
    ...
    if (-not $migrationCompleted) {
        if ($upgradeExistingMigratedEnvironment) {
            Write-Warning (
                "The target may contain a failed V49 forward migration. " +
