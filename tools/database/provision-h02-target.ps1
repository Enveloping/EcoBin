...
"@)
        ...
        if (
            $existingDomainTableCount -ne 93 -or
            $existingHistoryCount -ne 24
        ) {
            throw "Migrated resume requires the complete V24 target database"
        }
...
"@ | Out-Null
        ...
        Invoke-FlywayMigration -Target 24 -OwnerPassword $ownerPassword
        $migrationCompleted = $true
...
"@
    ...
    $historyCount = [int](Invoke-RootSql `
        -Database $DatabaseName `
        -Sql "SELECT COUNT(*) FROM flyway_schema_history WHERE success = 1;")
    if ($historyCount -ne 24) {
        throw "Expected twenty-four successful Flyway migrations"
    }
