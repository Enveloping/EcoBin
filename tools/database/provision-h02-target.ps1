...
"@)
        ...
        $resumeLayoutValid = (
            ...
            ($existingDomainTableCount -eq 118 -and
                ...
                $existingMaxVersion -eq 55)
        )
        if (-not $resumeLayoutValid) {
            throw (
                "Migrated resume requires a complete V30 through V55 " +
                "target database"
            )
        }
        if ($existingMaxVersion -lt 55) {
...
"@ | Out-Null
        ...
        Invoke-FlywayMigration -Target 55 -OwnerPassword $ownerPassword
        $migrationCompleted = $true
...
"@ | Out-Null
    ...
    if ($tables.Count -ne 118) {
        throw "Expected 118 domain tables, got $($tables.Count)"
    }
...
"@
    ...
    $historyCount = [int](Invoke-RootSql `
        -Database $DatabaseName `
        -Sql "SELECT COUNT(*) FROM flyway_schema_history WHERE success = 1;")
    if ($historyCount -ne 55) {
        throw "Expected fifty-five successful Flyway migrations"
    }
...
"@
    ...
    if (-not $migrationCompleted) {
        if ($upgradeExistingMigratedEnvironment) {
            Write-Warning (
                "The target may contain a failed V55 forward migration. " +
