...
"@)
        ...
        $resumeLayoutValid = (
            ...
            ($existingDomainTableCount -eq 99 -and
                ...
                $existingMaxVersion -eq 48)
        )
        if (-not $resumeLayoutValid) {
            throw (
                "Migrated resume requires a complete V30 through V48 " +
                "target database"
            )
        }
        if ($existingMaxVersion -lt 48) {
...
"@ | Out-Null
        ...
        Invoke-FlywayMigration -Target 48 -OwnerPassword $ownerPassword
        $migrationCompleted = $true
...
"@ | Out-Null
    ...
    if ($tables.Count -ne 99) {
        throw "Expected 99 domain tables, got $($tables.Count)"
    }
...
"@
    ...
    $historyCount = [int](Invoke-RootSql `
        -Database $DatabaseName `
        -Sql "SELECT COUNT(*) FROM flyway_schema_history WHERE success = 1;")
    if ($historyCount -ne 48) {
        throw "Expected forty-eight successful Flyway migrations"
    }
...
"@
    ...
    if (-not $migrationCompleted) {
        if ($upgradeExistingMigratedEnvironment) {
            Write-Warning (
                "The target may contain a failed V48 forward migration. " +
