...
"@)
        ...
        if (
            $existingDomainTableCount -ne 95 -or
            $existingHistoryCount -ne 25
        ) {
            throw "Migrated resume requires the complete V25 target database"
        }
...
"@ | Out-Null
        ...
        Invoke-FlywayMigration -Target 25 -OwnerPassword $ownerPassword
        $migrationCompleted = $true
...
"@
    ...
    $historyCount = [int](Invoke-RootSql `
        -Database $DatabaseName `
        -Sql "SELECT COUNT(*) FROM flyway_schema_history WHERE success = 1;")
    if ($historyCount -ne 25) {
        throw "Expected twenty-five successful Flyway migrations"
    }
