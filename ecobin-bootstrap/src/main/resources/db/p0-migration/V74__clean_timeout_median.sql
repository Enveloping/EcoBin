-- P1AT: usable timeout medians are real cleaning weights, not stable means or faults.
-- Preserve existing non-median and terminal-failure branches; reject NULL median quality.
-- One atomic CHECK replacement; no backfill, new tables, funds or runtime grants.
ALTER TABLE dev_physical_result
    ...
    ADD CONSTRAINT ck_dev_result_clean_pre_measurement CHECK (
        ...
        OR COALESCE((
            ...
            AND clean_pre_measurement_uid REGEXP
                '^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'
    ...
    ADD CONSTRAINT ck_dev_result_clean_final_measurement CHECK (
        ...
        OR COALESCE((
            ...
            AND clean_final_measurement_uid REGEXP
                '^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'
