...
本节是 2026-09-15 形成的新仓库候选，**没有热替换到当前v41卡**，也没有复用前文任何一次
...
执行投递或清运HIL。后续必须从包含本候选的新提交构建下一镜像，不能把当前v41卡的旧热修状态
记成已经继承。
...
| UART按请求5秒超时、精确自动恢复，以及验收/远程维护/问题证据线程隔离 | 仅仓库候选；当前卡未热替换、未做本节HIL | 本次待提交源码及运行时载荷清单；下一镜像必须重新构建 |
