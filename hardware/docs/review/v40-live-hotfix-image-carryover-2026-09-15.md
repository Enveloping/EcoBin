...
本节是 2026-09-15 形成的新仓库候选，**没有热替换到当前v42卡**，也没有复用前文任何一次
...
执行投递或清运HIL。后续必须从包含本候选的新提交构建下一镜像，不能把当前v42卡的旧热修状态
记成已经继承。
...
复核通过。详见
[v43镜像与后台认可记录](../../../docs/operations/orangepi-v43-backend-allowlist-deployment-2026-09-15.md)。

本轮没有写卡或回读，当前v42卡未改变，也没有执行真实热点、注册、UART、HMI、RS485、称重、
摄像头、机构、OneNet、投递/清运或断电HIL。后台认可不能替代现场验证；若后续写入v43，仍须
按第8节完整复跑。
...
| v41 厂家动作时间包络、IPC/网页等待、工作查询节流及通过报告总览兼容 | 当前 v42 卡已包含；本机验收已完成，等待扫描厂家袋 | 已由v43从提交`244dd4ea`重新构建，尚未写卡 |
...
| UART按请求5秒超时、精确自动恢复，以及验收/远程维护/问题证据线程隔离 | 当前v42卡未热替换、未做本节真实HIL | 已进入提交`244dd4ea`和v43运行时/载荷/镜像；后台已认可，尚未写卡 |
