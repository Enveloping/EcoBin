...
2026-09-16继承状态：v45已包含本文列出的仓库修复，并进一步删除原生与旧兼容业务入口中
残留的历史满溢投影准入。v44因此被取代，不应写卡；当前可继续验收的镜像以
[v45记录](v45-distance-business-decoupling-2026-09-16.md)为准。
...
写卡完成后，项目负责人明确要求把 v41 加入后台。线上只读预检确认生产服务器仍为
`VM-0-16-ubuntu`，应用发布保持 `20260914064458-82e72e0dca16`，认可列表精确以
`hardware-runtime-20260914-40` 结尾且 v41 尚不存在；服务器文件元数据、运行中容器环境、
远程业务下发关闭状态、生产预检、秘密隔离、双容器健康、后端健康和回环入口全部正常。

正式脚本在应用前经过两轮独立审查并修复回滚失败传播、回滚后容器旧值验证、旧态健康基线及
单硬链接约束，最终受审摘要为
`c69fef97596ff7a23fca78cf735a0d164e7d291423d398aee33749a3c21d76b6`。部署取得独占锁，创建
root 私有 0600 备份，只原子替换一个认可列表键，然后重载运行秘密和既有应用。服务器
`runtime.env` 摘要从
`bd926796463c6f7c2ddb844861b7dfed3ff50147dc79310f92cd375c6a532f59` 变为
`7e47a871958d53de85abe65c7d4dd1bedb5a529663503cf377a4002b1b5a0456`；备份为
`/etc/ecobin/runtime.env.pre-v41-20260915T030320Z.frpDC6rb`，摘要与旧文件一致。

部署结果和独立只读复核均为 PASS：服务器文件与后端容器现在精确包含 v40 和 v41，既有认可
版本全部保留；应用 release、后端/Web 镜像均未改变，双容器 running/healthy，生产预检、后端
健康、Web 回环和秘密隔离通过，`businessReleaseRemoteDispatchEnabled=false`。本次没有更改
业务数据或验收记录，没有发起投递/清运，也没有开启远程业务程序下发。详细证据见
[v41 后台认可版本部署记录](../../../docs/operations/orangepi-v41-backend-allowlist-deployment-2026-09-15.md)。

至此 v41 已完成镜像离线审计、指定 TF 卡完整写入/回读和后台运行时认可；仍未完成新卡冷启动
...
### 5.14 仓库候选：按请求超时与UART所有者线程隔离

本节是 2026-09-15 形成的新仓库候选；在当时验证阶段**没有热替换到v42卡**，也没有复用前文任何一次
现场无动作查询或投递结果作为本节验证。原“距离最后一条MCU消息超过10秒即判通信故障”已从
生产主链删除；现在只有需要回复的请求完整写出后，连续5秒没有收到匹配回复才超时。查询按查询
编号、请求类型和目标启动号匹配；START、配置、二维码等控制命令按命令编号、摘要、目标启动号
和命令序号匹配。短写和写异常只作为本机写入失败，不能伪装成MCU未回复。

普通周期查询的后续精确回复可以证明该查询通道恢复，但START等关键命令仍独立等待。请求超时
造成的原业务永久保持失败且业务价值为零；迟到匹配回复或故障后新请求的匹配回复只在同一事务中
恢复准确的UART故障、写恢复事件并清除仍精确等于 `MCU_COMMUNICATION_UNAVAILABLE` 的阻断，
不恢复原订单、当前袋、皮重、余额或提现。自动恢复不等待原业务槽释放，其他活动槽、MCU忙碌、
称重、配置和故障条件继续独立阻止下一单。历史 `automaticRecovery=false` 故障仍可使用人工入口。

为防止香橙派本机慢任务制造UART空洞，本候选又完成三条线程隔离：

1. UART线程冻结同一MCU启动号的验收快照，专用工作线程执行摄像头、COS上传和回读，前台领取
   结果后才完成原云命令；
2. 远程维护OPEN/CLOSE的本地socket调用进入独立单槽工作线程；
3. 问题证据工作线程读取不可变快照和生成JSON，UART线程每轮只持久化最多10条结果。

新增工作线程都具有停止、唤醒和有界退出；它们不直接写业务数据库，关闭后迟到完成结果会被
丢弃。UART循环耗时分为串口轮询、数据库短事务和完成队列三类，超过阈值仅输出限频的本机
`UART_OWNER_STALLED` 日志，不创建MCU通信故障。新增的 `uart_request_tracker.py`、
`bounded_worker.py` 已进入运行时载荷清单，避免下一镜像只带调用方而漏带实现模块。

本轮请求/恢复/慢任务专项为 `300 passed, 29 skipped`。Python 3.11完整 `hardware/tests` 为
`4334 passed, 126 skipped, 1 failed, 5 subtests passed`；唯一失败是既有Windows强杀进程后SQLite WAL重开
触发 `SQLITE_IOERR_TRUNCATE`，单独不改代码重跑为 `1 passed`。这些结果不等于现场通过：本节
尚未记录新候选文件的热替换前后SHA-256，没有在真实HMI、RS485、机构或断电条件下验证，也没有
执行投递或清运HIL。后续必须从包含本候选的新提交构建下一镜像，不能把当时v42卡的旧热修状态
记成已经继承。

### 5.15 v43镜像构建、写卡与后台认可

第5.14节源码已提交为`244dd4eaf72476110df322ac22017fe86336c4ad`，并从该提交重新生成
`hardware-runtime-20260915-43`、`software-payload-20260915-43`和
`0.1.0-single-card.20260915.43`。两次独立候选构建的运行时归档、载荷锁和候选整镜像逐字节
一致；最终受控镜像只从v42受保护成品继承既定3个批准路径，其余53,464个条目和启动区来自v43
候选。断网、镜像只读、仅合成状态且无真实硬件访问的ARM64冒烟通过，镜像工具63项通过、8项
按环境跳过。

最终镜像大小为2,571,108,352字节，SHA-256为
`6c9a97e21dec1be8c6a5d2a96b735ab401cf44eb8393c008d3e64ef129325e2f`，已导出到Git忽略的受保护
本地制品目录。生产后台随后只在原认可列表末尾追加`hardware-runtime-20260915-43`，v40、v41、
v42及更早值全部保留；配置文件与运行中容器一致，服务健康，远程业务程序下发仍关闭，独立SSH
复核通过。详见
[v43镜像、写卡与后台认可记录](../../../docs/operations/orangepi-v43-backend-allowlist-deployment-2026-09-15.md)。

项目负责人后续插入TF卡并明确确认覆盖Windows磁盘1。受保护工具再次核对USB类型、非系统/启动
盘、序列号`121220160204`和31,268,536,320字节容量后，完成2,571,108,352字节整镜像写入、
强制刷新和同范围完整回读；回读SHA-256与源镜像上述摘要一致。当前卡已是v43，但尚未装卡冷启
动，也没有执行真实热点、注册、UART、HMI、RS485、称重、摄像头、机构、OneNet、投递/清运或
断电HIL。后台认可和写卡回读都不能替代现场验证，仍须按第8节完整复跑。

### 5.16 v44测距解耦、投递快速结束与新镜像

提交`25fd2d455eb6c6741a7111887747d1ea16788114`把超声波测距从投递/清运业务测量中移除，验收只读
设备事实快照且不要求测距新鲜；关门机械行程为3秒，用户继续/结束窗口仍为30秒。MCU在本轮
关门已接受后会保留一次提前到达的END/CONTINUE，并在本轮关门后重量形成时绑定和完成，修复
屏幕已回首页而热点页等待30秒的问题。UART推进rc.27、OneNet推进2.5.0、数据库迁移候选V84。

新发布为`hardware-runtime-20260916-44`、`software-payload-20260916-44`和
`0.1.0-single-card.20260916.44`。最终工厂验收镜像只从v43受保护成品继承既定3个批准路径；
其余53,464个文件系统条目和启动区来自v44无秘密候选。断网、镜像只读、无真实硬件访问的
ARM64冒烟通过。最终镜像2,571,108,352字节，SHA-256为
`503913146c5e76aca09c6dd9e91d7b1906487af5c488c1a3e74686ee095f79b4`。当前只完成制品归档，
没有沿用此前的写卡或后台授权：尚未写TF卡、未追加后台认可版本、未烧录MCU，也未执行真实HIL。
详细边界和证据见[v44记录](v44-acceptance-distance-and-delivery-timing-2026-09-16.md)。
...
| v41 厂家动作时间包络、IPC/网页等待、工作查询节流及通过报告总览兼容 | 当前v43卡已包含；此前本机验收已完成，仍须在v43冷启动后复验 | 已由v43从提交`244dd4ea`重新构建并写卡回读 |
...
| UART按请求5秒超时、精确自动恢复，以及验收/远程维护/问题证据线程隔离 | 当前v43卡已写入但未冷启动、未做本节真实HIL | 已进入提交`244dd4ea`和v43运行时/载荷/镜像；后台已认可，写卡及完整回读通过 |
...
2. 新提交包含 `780d2ecb`、`5bf89e67`，以及第 4 节全部源码、unit、维护安装器内嵌基线和
   ...
   本节新增候选还必须包含 `uart_request_tracker.py`、`bounded_worker.py` 及同步后的运行时
   载荷清单。
...
13. 在不发送业务动作的前提下连续查询并跨过10秒空闲，确认不会仅因静默产生UART通信故障。
14. 受控丢弃一条已完整写出的请求回复，确认不足5秒不超时、达到5秒才失败；随后放行准确迟到
    回复，确认只恢复通信，原失败业务、袋状态和业务价值均不恢复。
15. 让摄像头/COS或远程维护socket阻塞超过30秒，确认UART启动号和设备事实查询持续收发，日志
    只记录本机线程耗时，不能把香橙派调度延迟报成MCU故障。
16. 投递可使用无业务价值的受控验收场景验证上述边界；清运仍必须另做有人值守的真实HIL，不能
    用投递结果替代。
