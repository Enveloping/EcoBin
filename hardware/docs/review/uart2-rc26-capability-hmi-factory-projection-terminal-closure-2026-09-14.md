...
项目负责人明确确认覆盖Windows磁盘1。2026-09-14 13:21—13:25（Asia/Shanghai），受控写卡
工具在写入前重新确认目标为USB、非系统盘、非启动盘、序列号`121220160204`、容量
31,268,536,320字节且没有Windows挂载盘符；源镜像长度和SHA-256也再次匹配第8节记录。

工具向该卡实际写入2,571,108,352字节，强制刷新后从物理磁盘起始位置回读相同字节数。回读
SHA-256为`94e1bf8314310379c579ea469ae742949b7c121fefd8c6455ab0ca9869f2b84d`，与源镜像完全一致，
结果为`PASS`。证据归档于
`hardware/image-artifacts/evidence/hil-uart2-rc26-v40-card-write-20260914-40/`。

该结果只证明实验TF卡前2,571,108,352字节与v40候选镜像一致。写卡时没有连接香橙派或MCU；
...
生产服务器最初只读检查确认后端容器为`running/healthy`，但实际认可列表尚无
`hardware-runtime-20260914-40`。项目负责人随后明确授权追加该版本。部署事务以线上精确旧值
为前提，只追加v40，保留v37和全部既有认可版本；修改前建立root私有备份，执行并发修改防护、
生产预检、运行秘密重新暂存、应用reload和健康等待。独立复核确认服务器文件和运行中容器均已
包含v40，远程业务下发仍关闭，后端/Web镜像ID与修改前一致。部署记录见
[v40后台认可列表部署记录](../../../docs/operations/orangepi-v40-backend-allowlist-deployment-2026-09-14.md)。

2026-09-14 15:40—15:45（Asia/Shanghai），受控写卡工具在管理员进程内再次核对Windows磁盘1
为USB、非系统/启动盘，序列号`121220160204`、容量31,268,536,320字节。实际写入和完整范围
回读均为2,571,108,352字节；回读SHA-256为
`21581060db890b0902e2cc270e44b23e1867f6e7bdd8db8eeac44948b258690a`，结果`PASS`。
写卡时没有连接香橙派或MCU，因此真实热点、注册、UART、HMI、RS485、机构和断电测试仍需在
装卡启动后逐项验证。受控HIL镜像不是正式签名生产镜像。
