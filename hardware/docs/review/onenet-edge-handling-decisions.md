...
> [!IMPORTANT]
> 2026-08-13 补充 fixed-frame 屏幕刷新：香橙派在工作槽空闲、串口打开时默认每 60 秒
> 从 SQLite 重新读取并发送当前 URL；投递/清运进行中暂停，结束后最多约 5 秒恢复。
> 这只是无应答 `A0` 屏幕刷新，不新建 OneNet 命令、不改变 V42 验收事实，也不重放
> 物理命令。当前静态 URL 没有防重放能力；未来动态 URL 必须由后端验证短期签名，不能
> 信任设备附加的裸时间戳。

> [!IMPORTANT]
> 2026-08-24 V57 冻结跨机器时钟边界：设备或微信返回的时间只作为原始诊断证据，不能
> 与后端数据库时间比较后决定是否接收、回滚事务或排列业务事实。`SYNCED` 事件必须携带
> `occurredAt`；`ESTIMATED/UNAVAILABLE` 事件必须令该字段为 `null`，香橙派另在本地保存
> 原始墙钟读数。业务顺序使用稳定 UID、设备序号和后端接收时间。设备时间比后端快超过
> 60 秒只告警，不拒绝；迟到积压同样允许收敛。香橙派先请求 NTP 服务自愈，持续 5 分钟
> 仍未同步才上报 `CLOCK_UNSYNCED` 警告，期间投递、清运、照片、远程维护和封存均继续。
...
不可变设备名。香橙派回传以下可重复验证的功能证据：

1. OneNet 已连接，SQLite 可读写，配置持久化健康；时钟健康作为诊断证据，不再决定验收是否通过；
2. MCU 通信和传感器检查通过；`mcuSimulated` 只记录来源，不影响结果；
...
验收事实；模拟 MCU 或模拟摄像头本身不再构成失败。租户分配、机构分配仍分别由平台和
租户执行一次，且之后不可改派。
...
4. OneNet 返回 `10410` 时，记录 `TARGET_NOT_FOUND` 并把任务阻断为
   ...
   “暂时离线”处理。
5. 只有收到 `deviceOnline` / `deviceOffline` 生命周期通知后，后端才更新传输状态并重新
   计算任务闸门；普通设备消息和下行错误都不改写权威状态。

OneNet 生命周期通知经既有北向 Pulsar 链路进入，不增加公网回调。后端不发送空命令或
周期性服务调用来探测在线，也不在状态未知时下发配置探测。确认边缘事件的自动尝试上限为
100 次。
...
### 2.5 V57 时钟质量、自愈与降级运行

状态：**已确认并于 2026-08-24 实现**

香橙派每次形成一个事实时只采样一次系统时钟和同步状态，避免同一事实混用两个时刻：

- 操作系统确认 NTP 已同步时，使用 `clockQuality=SYNCED` 并发送 UTC `occurredAt`；
- 时间数值看似合理但同步状态未确认时，使用 `ESTIMATED`；系统纪元明显无效时使用
  `UNAVAILABLE`；后两种情况的云端 `occurredAt` 固定为 `null`；
- 本地 SQLite 继续保存原始墙钟读数和时钟质量，不能通过钳制、改写成后端时间或伪装
  `SYNCED` 来通过校验。

发现未同步后，运行时立即尝试启用并重启当前已安装的 `chrony/chronyd` 或
`systemd-timesyncd`，以后最多每 5 分钟重试一次；禁止直接执行 `date -s`。连续 5 分钟
仍未同步才建立 `CLOCK / CLOCK_UNSYNCED / WARNING`，恢复同步后发送匹配恢复事实。
该警告不阻止任何新旧业务，也不回滚已经可靠保存的命令或事件。

本地时钟不可信时，香橙派仍校验命令签名事实、稳定身份、摘要、字段形状以及
`issuedAt → expiresAt` 的允许时长，但不拿本地墙钟判断命令、远程维护会话、COS 授权或
照片保留期是否已经到期；这些期限由后端和外部服务的可信时钟继续执行。恢复 NTP 后，
新的本地期限判断自然恢复，既有稳定 UID 和序号保证不会因此重复执行物理命令。

后端保存设备/微信原始时间时，不再与 `created_at`、`received_at` 或
`backend_received_at` 做跨时钟大小比较。业务状态和排序只使用后端接收时间、数据库行号、
稳定任务/事件身份或同一外部权威内可比较的时间。设备时间比后端快超过 60 秒只产生脱敏
警告且不得参与机构归属等业务裁决；过去很久的离线积压则按正常可靠事实接收。
...
> **2026-08-09 修订（取代本节原有兼容皮重规则）**：固定帧协议现已具备
> `F0 01 F0` / `F1 ... F1` 实时传感器查询。`measureEmptyBagBaseline` 必须为本次
> 命令发送一次 F0，并且只使用随后收到的 F1 重量；禁止再复用 `DD.POST`、
> `EF.POST`、启动自检缓存或 0 克默认值。重量有效时保存当前袋皮重并上报稳定结果；
> F1 超时、格式错误或重量无效时上报明确失败，投口继续阻断。F0/F1 的当前线缆格式和
> 时序以《单片机-香橙派适配通信协议详细内容》为准。下面 3.7.1～3.7.6 保留为历史
> 决策背景，不再是当前实现要求。
...
> **2026-08-09 修订（取代本节原有历史重量/0 克兼容来源）**：fixed-frame
> `baselineMeasurementComplete` 只允许由本次皮重命令触发的新 F0/F1 查询产生。
> 成功事件中的重量来自本次合法 F1；查询或重量失败仍产生带真实失败状态的完成事件，
> 但不建立袋皮重。不得使用旧投递、旧清运、旧自检或默认 0 克构造成功事件。下面
> 4.6.1～4.6.7 记录的是 F0/F1 能力加入前的历史方案，凡与本修订冲突之处均已废弃。
...
变化时由 MCU 主动发送。香橙派在健康状态下不周期轮询；启动 F0 没有取得完整健康证据
时，才由现有命令消费者在无作业占用时按 5、10、20、40、60 秒退避重查；间隔从上次
查询结束后计算，之后最多每 60 秒一次。作业槽被占用时顺延 5 秒且不推进退避档位。香橙派不把 DD/EF 的 FULL 推断成
烟雾，也不把“未收到异步 CC”推断成故障；只有一次实际 F0 查询超时或收到非法 F1，
才形成相应的 TIMEOUT/PROTOCOL_ERROR 当前事实。
...
连接 OneNet、上报运行快照和可靠故障事实。自动重查只有在以下证据全部成立后停止：
F1 查询成功且通信正常、`VALID=3`、重量为 0～350000 克整数、红外值合法，并且最新
烟感为 `NORMAL/OK` 或 `ALARM/OK`。`ALARM/OK` 表示通信和烟感读取有效，因此停止 F0
重查，但安全准入仍阻止新的投递和清运。CC 可以更新最新烟感，不能单独替代 F0/F1 对
通信、重量和红外的证明。

合法 F1 一旦证明通信恢复，香橙派精确关闭活动的 `UART/UART_PROTOCOL` 故障；若重量、
红外或烟感证据仍不完整，则保持业务阻断并继续退避重查。相同失败只增加故障发现次数并
刷新最后发现时间；相同烟感投影不再产生新的 MCU inbox 和可靠安全事件。真正的烟感
状态变化仍按本节形成一次事件，UART 恢复只形成一次
`FIXED_FRAME_SELF_TEST_RETRY_SUCCEEDED` 恢复事实。重查不触碰历史失败命令，不重放
开门、投递或清运等物理命令，也不改变 uart-v1、OneNet 或后端契约。
...
- 启动自检失败后由命令消费者串行调度 F0，使用 5/10/20/40/60/60 秒退避；
- 健康状态不查询，作业占用时跳过，uart-v1 不启用该恢复控制器；
- 合法 F1 无需重启即可关闭 UART 故障；通信恢复但传感器证据不全时继续重查；
- 相同自检投影在适配器入队前抑制，不再为每次超时生成 MCU inbox；同一活动故障只刷新
  `discovery_count/last_detected_at`，真正恢复只生成一个可靠恢复事件；
- 运行快照在串口打开但活动 `UART_PROTOCOL` 故障仍存在时报告 `FAULT`，故障关闭后报告
  `READY`，串口关闭时报告 `DISCONNECTED`；
- 历史 `FAILED` 命令保持终态，恢复控制器没有任何物理命令重放路径。
- 物理准入会同时检查串口是否打开及活动 `UART_PROTOCOL` 故障；历史正常 F1 不能覆盖
  当前通信故障，也不能让新投递或清运进入 MCU 写入阶段。
...
2026-08-13 Windows 开发机 Python 3.11 硬件全量结果为
`274 passed, 9 skipped, 5 subtests passed`。
...
3. 运行状态发生变化时请求生成，5 秒内多次变化合并为一份；
4. 正常在线期间默认每 5 分钟生成一次；
5. 多份快照等待发送时只保留最新一份，不补发已经过时的历史快照。
...
- 后端不按快照年龄把设备标记离线；没有任何可信快照时才阻止依赖运行事实的业务；
- 单次快照丢失不产生可靠故障事件。
