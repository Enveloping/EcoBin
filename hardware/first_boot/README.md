...
完成并同时存在受保护的远程维护凭据和 `ecobin-remote` 低权限账号后，只对该账号额外放行
凭据中配置的跳板 SSH 端口；普通登录账号、硬件主进程和其他本机进程仍不能使用该 SSH
出口，任何接口也不获得入站 SSH 例外。规则检查、应用或事后 JSON 验证任一步失败，都会
恢复并重新证明紧急全拒绝规则。已资格化的 H616/Air780E 路径可能把新 SSH SYN 标记为
`invalid`，因此这条账号和端口双重受限的规则必须位于 `invalid-output` 丢弃规则之前；
不得借此移动或删除面向其他进程的异常流量丢弃边界。
