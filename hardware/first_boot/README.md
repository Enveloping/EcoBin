...
- P7的私有状态与最终报告，并要求报告同时绑定当前发布、硬件配置、UART v2 MCU固件身份及能力；
...
1. 用`QUERY_DEVICE_IDENTITY`核对当前MCU启动编号、固件身份、最高命令序号和能力位；必要能力
   掩码为`0x8100`，不能按版本字符串猜测能力；
2. 应用原生配置并用`QUERY_DEVICE_FACTS`核对同一当前启动、配置摘要、空闲控制状态和实际采集
   数据。空载、加载本轮参考重量、取下三个阶段的样本、稳定值、差值和判定全部进入报告；参考
   值在网页按整数克填写（默认500g，可用400g、1000g或其他允许值），该值只定义本轮比较标准，
   不校准硬件；
3. 两个角色分别从固定 `/dev/v4l/by-id` 候选中选择摄像头：外摄首选
...
5. 投递和清运只验证本机硬件，不建单、不写生产数据库、不上传图片、不访问后端。每项只写出
   ...
   `WORK_RESULT`，可靠保存后发送`RESULT_SAVED`并核对回复。投递动作后由操作员确认机构和区域
   安全；清运动作后独立确认门已关闭；
6. 完整页面、初始化、实时重量或二维码指令组原子进入UART3软件发送队列即视为已显示；不等待
   USART完成、串口屏回执或屏幕读回。队列不足时零字节发布并重试，现场未显示按HMI工程、接线
   或屏幕硬件故障处理；
7. 所有可能已经写出的START均不会构造第二条命令。掉电或结果不确定时保持
   `RECOVERY_REQUIRED`；重启后只查询原命令和原结果，不据按钮、过程重量或动作片段重建成功。

注册完成后，`ecobin-factory-handoff.service`独占同一UART锁，取得当前原生身份和设备事实，
再与P7报告逐项比较；只有当前启动、固件、能力和配置事实匹配才原子写入
...
UART安全交接还会把本地PASSED报告中的`mcuRemoteUpdateCapable`与报告摘要原子写入
`/var/lib/ecobin/device-capabilities.json`。正式运行时只信任这个文件：值为 `false`
不阻止本地业务、OneNet或封存，但MCU升级命令会在下载固件或建立维护锁前
...
- 冷启动热点、网页、UART5以及实际安装时的BOOT0（物理7号脚/wPi 2）和NRST（物理11号脚/wPi 5）；
- 当前原生身份/`0x8100`能力、配置/设备事实、本轮参考重量与全部样本、满溢和烟感显示、投递/
  清运动作、HMI、投递区域安全确认和清运门关闭确认，以及人为断电后保持恢复锁且不构造第二条
  START的安全恢复；
