...
2. 空载、加载已知重量测试物、取下三个阶段都要求连续 3 个样本、最大离散 2g；参考值在
   网页按整数克填写（默认 500g，可用 400g/1000g），加载减空载相对参考值误差为 ±10g，
   取下须恢复空载 ±10g；生成、首启、交接和封存共用该规则，不再只接受 500g 报告；
...
- 已知重量测试物（默认 500g，并复核 400g/1000g）、红外、舱门、投递/清运动作，DD 后投递区域安全确认、EF 后清运门关闭确认，以及
