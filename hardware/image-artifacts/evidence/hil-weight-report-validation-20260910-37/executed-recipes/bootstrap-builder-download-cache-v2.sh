...
PY
    ...
    target_deb_directory="/ecobin-cache/target-debs"
    mkdir -p -m 0755 -- "${target_deb_directory}"
    (
...
# Preserve signed snapshot metadata in the host cache.
...
if [[ "${build_mode}" = software-payload ]]; then
    exec bash /builder-payload-cache-v2.sh "$@"
fi
