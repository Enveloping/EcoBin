...
本轮尚未写入 TF 卡。当前结果只证明本地 v10 镜像构建完成且离线预检通过，不证明任何
介质已经获得这些字节，也不证明首次启动行为已经复测。写卡前仍须重新检测目标磁盘并由
项目负责人明确确认磁盘号和介质序列号；写入后须完成同范围 SHA-256 回读，再装回香橙派
从空白状态重走局域网页验收、自动注册、机器验收/封存、租户与机构分配和业务准入流程。
