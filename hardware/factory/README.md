...
`RECOVERY_REQUIRED`，不会自动重发。清运 EF 完成后还必须复位并再次验证原 F3 身份、F1 自检
和串口静默，才进入“等待确认舱门关闭”；这个确认是独立第二步，不能在动作前预先勾选。恢复
失败时保持锁定，阻止正式业务抢占一个状态未知的 MCU。
