...
报告生成、首次启动、UART 交接及封存校验共用本轮参考值规则，不能在下一步重新限定为
500 g。安装配置中的 500 g 保留为兼容默认值及硬件配置摘要的一部分；改本轮参考值应在
网页采集空载前填写，不改 `hardware.env`。例如实际放置 500 g、设备只增加 28 g 时，
仍须填 500 g 并判定失败，不能把参考值改成 28 g 来代替校准。后端只校验报告摘要与授权
绑定，没有另一个 500 g 砝码门禁；本机不合格报告仍阻止交接和封存。

网页显示空载、加载、取下的稳定值、中位数采样规则、参考范围、重量差、偏差、各阶段最后
最多 32 次有效 F1 重量回复及有效回复总数。连续不稳定或后续查询超时也保留已获得的数值；
...
6. 实测 UART5 revision 2、已知参考重量±10g（默认 500g，并复核 400g/1000g）、双摄像头角色、STM32 `0x0410` 只读 ROM 探测、投递和
