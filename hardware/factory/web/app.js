...
const DETAIL_DESCRIPTIONS = Object.freeze({
  ...
  CLOUD_ACCEPTANCE_FAILED: "后台已明确判定本次验收未通过，不会继续等待封存授权；请在后台查看失败项目。",
...
const ERROR_DESCRIPTIONS = Object.freeze({
  ...
  CLOUD_ACCEPTANCE_FAILED: "后台已判定本次验收未通过；请在后台查看失败项目，修复后重新发起验收。",
  ...
  MCU_FACTORY_WORK_STILL_RUNNING: "控制板仍在执行上次硬件动作；请等待机构停止后刷新状态。",
  MCU_FACTORY_WORK_FAILED: "控制板报告本次硬件动作失败；请检查机构和现场安全状态。",
});
...
const ACTIONS = Object.freeze({
  ...
  RUN_DELIVERY: { label: "执行投递硬件测试", prompt: "将真实驱动投递机构。完成投递并关门后，请在设备屏幕选择结束投递；系统随后会收取结果。确认周围无人、机构无阻挡？", parameters: () => ({ operatorAreaSafeConfirmed: true }) },
...
const ACTION_INSTRUCTIONS = Object.freeze({
  ...
  RUN_DELIVERY: "操作会真实驱动机构；完成投递并关门后，请在设备屏幕选择结束投递。开始前确认人员和障碍物已经离开。",
