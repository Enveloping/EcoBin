...
public class StaffOrganizationUserBindingController {

    private final TargetOrganizationUserBindingService bindingService;

    public StaffOrganizationUserBindingController(
            TargetOrganizationUserBindingService bindingService) {
        this.bindingService = bindingService;
    }

    @PostMapping(
            "/organizations/{organizationCode}/organization-user-lookups")
    public ResponseEntity<TargetApiEnvelope<OrganizationUserLookupView>>
        ...
        return noStore(bindingService.lookupByPhone(
                tenantCode(),
                organizationCode,
                body.phoneNumber()), request);
    }

    @GetMapping(
            "/organizations/{organizationCode}/staff-accounts/{staffUid}"
                    + "/miniapp-binding")
    ...
    public TargetApiEnvelope<StaffMiniappBindingView> revoke(
        ...
        String organizationCode =
                bindingService.organizationCodeForCurrentTenantBinding(
                        bindingUid);
