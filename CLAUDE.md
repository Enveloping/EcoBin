...
- 当前仓库已由 F-03 收口为最终九模块 reactor。独立目标数据库迁移已推进到 V52：V51 以前保留既有 99 张领域表，V52 新增注册、厂家操作员验收小程序、袋标签占用和远程维护事实，共 112 张领域表、76 条有效权限定义。服务器实际版本仍必须在部署前现场核对并用 H-02 续跑到 V52，不能仅凭旧记录假定已经升级。
...
4. 判断“系统现在如何运行”时，以当前代码、测试、根 `pom.xml`、旧运行 Flyway V1～V14、独立目标迁移 `db/p0-migration/V1～V52`、当前 OneNet v2 物模型和设备程序为准。
...
| 数据库 | 旧栈恢复单元仍是 V1～V14/13 张主要表；目标 V1～V52 为 112 张领域表，迁移只由独立 Maven 作业执行，新运行制品不含 Flyway 运行库或迁移脚本，并以固定 V1 marker + V52 guard 检查；服务器实际版本部署前必须现场核对并补齐到 V52 | 新旧应用/数据库成对隔离；V36～V51 保留既有永久资产、共享小程序、设备契约和资金状态演进；V52 增加设备自注册、厂家验收小程序、初始袋标签占用及按需反向 SSH |
...
3. 新数据库 V1～V52 的 DDL/Flyway、种子数据、数据库账号和旧新应用/数据库成对切换；
