...
- 接口设计编号为 I-001～I-055。DD-004 与 PDD-001 已分别写回 I-051～I-053；29 项正式任务已经发布到 [`docs/planning/tasks/p0-controlled-loop/`](docs/planning/tasks/p0-controlled-loop/00-index.md)。项目负责人已授权并完成 H-01、H-02、F-01～F-11、V-01；V-02 与 H-03 为 `ready` 但尚未获得实施授权。F-11 的 SQLite v4、OneNet 命令受理、固定帧 MCU 适配、照片/COS 和故障自动测试已按当前软件范围转为 `done`；后续真实香橙派、OneNet 或真机验证发现范围内问题时重新打开。现有单片机无需原生实现 UART 1.0；不支持的 MCU 功能按本地保存、明确失败或未知占位降级，真机验收归 H-03。当前合计 `done` 14、`ready` 2、`in-progress` 0、`blocked` 13。
...
- F-11 已完成 SQLite v4、OneNet 命令可靠受理、固定帧 `AA/BB/EE` 与 `DD/EF` 适配、
  ...
  硬件套件为 `165 passed, 5 subtests passed`，契约套件为
  `43 passed, 64 subtests passed`。F-11 已按负责人接受的当前范围转为 `done`；
  后续验证发现范围内问题时重开，固定帧真机验收属于已就绪但尚未授权的 H-03；
...
- 当前实施授权按任务范围管理：H-01、H-02、F-01～F-11、V-01 已授权并完成；
  H-02 已完成本地开发演练和服务器阶段 0～3；阶段 4、seed、旧库操作和真实入口切换
  仍须分别获得明确授权；V-02、H-03 为 `ready` 但未获实施授权；
