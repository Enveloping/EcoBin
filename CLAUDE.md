...
- 当前仓库已由 F-03 收口为最终九模块 reactor：OneNet/COS/微信外部实现位于 integration，旧 system 已迁入 identity，旧 business 的旧行为已分别迁入 funds、recycling、operations，并通过 device 公开端口协作。数据库 V1～V21 已包含投递配置与清运正常流程的前向迁移，但尚未接管旧应用运行库；V20 已删除废止的清运审核列/约束并以 `rec_clean_record_change` 替换 `rec_clean_revision`，目标业务表数仍为 83。设计文档“已冻结”和完整 DDL 已具备，不表示真实环境供应或设备协议已经整体完成。
