...
- 当前仓库已由 F-03 收口为最终九模块 reactor。独立目标数据库迁移已推进到 V47：V35 的 99 张领域表经 V36～V38 收口永久设备资产和机器验收，V39 把机构独占 AppID/OpenID 改为共享小程序渠道、微信主体和独立机构账号，V40 补齐永久资产事件目标，V41 删除渠道/机构级二维码入口并改用全局固定地址，V42 增加设备入口 URL 边缘下发进度和验收证据，V43 增加防伪袋码打印历史，V44 为 Web 清运操作查询增加机构/状态时间索引，V45 增加清运设备列表活动索引和失效事件终态隔离原因，V46 增加平台全局运行快照策略和配置滚动下发版本，V47 对齐可靠设备任务终态和投递、清运、初始皮重业务代际，共 99 张领域表、76 条有效权限定义。服务器实际版本仍必须在部署前现场核对并用 H-02 续跑到 V47，不能仅凭旧记录假定已经升级。
...
4. 判断“系统现在如何运行”时，以当前代码、测试、根 `pom.xml`、旧运行 Flyway V1～V14、独立目标迁移 `db/p0-migration/V1～V47`、当前 OneNet v2 物模型和设备程序为准。
...
| 数据库 | 旧栈恢复单元仍是 V1～V14/13 张主要表；目标 V1～V47 为 99 张领域表，迁移只由独立 Maven 作业执行，新运行制品不含 Flyway 运行库或迁移脚本，并以固定 V1 marker + V47 guard 检查；服务器实际版本部署前必须现场核对并补齐到 V47 | 新旧应用/数据库成对隔离；V36 是破坏式设备模型切换，V37/V38 是其前向修订，V39 是共享小程序身份模型切换，V40/V41/V42 收口设备事件、二维码入口和 URL 下发，V43 增加平台防伪袋码打印历史，V44 增加清运操作 Web 查询索引，V45 增加清运设备列表活动索引和失效事件终态隔离原因，V46 增加平台全局运行快照策略和配置滚动下发版本，V47 对齐设备可靠任务和业务代际终态 |
...
3. 新数据库 V1～V47 的 DDL/Flyway、种子数据、数据库账号和旧新应用/数据库成对切换；
