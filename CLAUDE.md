...
- 当前仓库已由 F-03 收口为最终九模块 reactor：OneNet/COS/微信外部实现位于 integration，旧 system 已迁入 identity，旧 business 的旧行为已分别迁入 funds、recycling、operations，并通过 device 公开端口协作。数据库 V1～V31 已包含设备、投递、清运和资金闭环的前向迁移；服务器独立目标空库已于 2026-08-04 从 V10 重建到 V31，当前为 96 张领域表、77 条权限定义、其余业务数据 0，但尚未承接旧应用或权威业务写入。设计文档“已冻结”和完整 DDL 已具备，不表示应用入口或设备协议已经整体切换。
...
4. 判断“系统现在如何运行”时，以当前代码、测试、根 `pom.xml`、旧运行 Flyway V1～V14、独立目标迁移 `db/p0-migration/V1～V31`、当前 OneNet 物模型和设备程序为准。
...
| 数据库 | 旧栈恢复单元仍是 V1～V14/13 张主要表；目标 V1～V31 的 96 表迁移只供独立 Maven 作业使用，新运行制品不含 Flyway 运行库或任何迁移脚本，并以固定 V1 marker + V31 guard 在 MySQL 8.4 Fake 空业务库通过验收；服务器目标库已是同一 V31 空库，但尚未切换旧栈 | 新旧应用/数据库成对隔离；H-02 已供应正式身份和环境，不导入旧业务数据，后续应用部署与权威写入切换仍由独立门控制 |
...
3. 新数据库 V1～V31 的 DDL/Flyway、种子数据、数据库账号和旧新应用/数据库成对切换；
