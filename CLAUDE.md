...
- 截至 2026-08-07，需求、P0 范围、业务模型、系统架构、目标数据库、目标接口和详细设计均已完成确认；资金侧已有 D-046/I-056，设备侧新增并冻结 D-047/I-057。
...
- 当前仓库已由 F-03 收口为最终九模块 reactor。独立目标数据库迁移已推进到 V38：V35 的 99 张领域表经 V36 删除旧设备模型并新增验收/出厂袋事实，V37 补齐平台验收确认任务，V38 将模拟来源改为仅诊断，仍为 93 张领域表、76 条有效权限定义。服务器实际版本仍必须在部署前现场核对并用 H-02 续跑到 V38，不能仅凭旧记录假定已经升级。
...
4. 判断“系统现在如何运行”时，以当前代码、测试、根 `pom.xml`、旧运行 Flyway V1～V14、独立目标迁移 `db/p0-migration/V1～V38`、当前 OneNet v2 物模型和设备程序为准。
...
| 数据库 | 旧栈恢复单元仍是 V1～V14/13 张主要表；目标 V1～V38 为 93 张领域表，迁移只由独立 Maven 作业执行，新运行制品不含 Flyway 运行库或迁移脚本，并以固定 V1 marker + V38 guard 检查；服务器实际版本部署前必须现场核对并补齐到 V38 | 新旧应用/数据库成对隔离；V36 是无生产设备数据前提下的破坏式设备模型切换，V37/V38 是其前向修订，不迁移旧部署数据 |
...
- 普通用户按机构 AppID/OpenID 建立机构身份，钱包不能跨机构。首次 `wx.login` 创建用户时记录注册时间和可空来源永久设备资产，并在同一事务创建零余额钱包；后续扫码不回填来源，重复登录不补建或重建钱包。
...
3. 新数据库 V1～V38 的 DDL/Flyway、种子数据、数据库账号和旧新应用/数据库成对切换；
