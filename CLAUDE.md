...
- 接口设计编号为 I-001～I-055。DD-004 与 PDD-001 已分别写回 I-051～I-053；29 项正式任务已经发布到 [`docs/planning/tasks/p0-controlled-loop/`](docs/planning/tasks/p0-controlled-loop/00-index.md)。项目负责人已授权并完成 H-01、F-01、F-02、F-03、F-04、F-05、F-06、F-07；F-08 已完成 Fake 可靠任务 tracer 和 MySQL 8.4 验收，处于 `in-review`；F-11 已获授权并完成 `APPLY_CONFIGURATION` 软件纵切，继续处于 `in-progress`；F-09 与 H-02 已因前置完成进入 `ready`，但 F-09 编码和 H-02 真实环境操作仍须分别授权。F-10 软件机器来源、MCU Registry checkpoint 和通用 Java/Python 3.11/C11 黄金样本已完成，仍缺 MCU 实际工具链与 HIL，任务级保持 `blocked`。当前合计 `done` 8、`ready` 2、`in-progress` 1、`in-review` 1、`blocked` 17。
...
| 数据库 | 旧栈恢复单元仍是 V1～V14/13 张主要表；目标 V1～V10 的 83 表迁移只供独立 Maven 作业使用，新运行制品不含 Flyway 运行库或任何迁移脚本，并以固定 V1 marker + V10 guard 在 MySQL 8.4 Fake 空业务库通过验收，但尚未切换旧栈 | 新旧应用/数据库成对隔离；H-02 供应正式身份和环境，后续纵向任务迁移目标业务，不导入旧业务数据 |
...
| IoT/UART | 当前 OneNet 字段、匿名 IoT 入口、临时 AA/BB/CC/DD 与旧 D1 混合 | OneNet 可信 inbox/业务确认、COS 受限直传、SQLite 恢复和唯一 UART 1.0 二进制协议 |
...
- 清运门只有电磁阀通断、没有门磁或关门执行器；第一次可能解锁即为不可逆边界，再次解锁沿用原操作，完成必须由清运员人工关门确认，并已有最终稳定重量或明确终态称重故障。通断仅推定门状态，`SAFE_CLOSE` 不适用于清运门。恢复中仅断电不能释放整机占位；断电且原清运员现场确认门扇关闭后可以释放整机占位，但原投口、操作和袋预留继续锁定。
...
- UART 1.0 使用 `0xEC42`、最大 256 字节、big-endian、CRC-16/CCITT-FALSE、HELLO、ACK/NACK、幂等命令和 MCU 启动代际事件。旧 D1 与临时 AA/BB/CC/DD 在正式切换时整体退出，生产不保留失败后回退旧协议的双解析器。
- 香橙派目标 Python 3.11；不要使用开发机 Python 3.14 专属语法。MCU/香橙派任一端重启都先查询和恢复真实状态，绝不自动重放旧开门。
...
- OneNet 已完成设备 MQTT 联通；COS 仍待 F-11 完整上传闭环；香橙派 `/dev/ttyS5`
  可打开，但真实 MCU 当前没有回应 UART 1.0 `HELLO`；
- MCU 固件由其他负责人实施，仍需按 UART Registry 补齐实际 MCU C 工具链和 HIL；
...
- 当前实施授权按任务范围管理：H-01、F-01、F-02、F-03、F-04、F-05、F-06、F-07 已授权并完成，
  F-10 软件已完成，F-08 已授权实施并处于 `in-review`，F-11 已授权且处于 `in-progress`；
  F-09 为 `ready` 但未获编码授权，H-02 为 `ready` 但未获真实环境操作授权，不能据此前置进展自动实施。用户说“讨论、计划、设计”时保持文档级工作；
