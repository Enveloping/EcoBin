...
- 当前仓库已由 F-03 收口为最终九模块 reactor：OneNet/COS/微信外部实现位于 integration，旧 system 已迁入 identity，旧 business 的旧行为已分别迁入 funds、recycling、operations，并通过 device 公开端口协作。数据库 V1～V10 共 83 张表的历史独立迁移已经过 MySQL 8.4 双空库验证，但尚未接管旧应用运行库；其中仍含已废止的清运审核结构，待 V19 前向删除审核列/约束并以 `rec_clean_record_change` 替换 `rec_clean_revision` 后，目标业务表数仍为 83。设计文档“已冻结”和完整 DDL 已具备，不表示该前向迁移、纵向业务、真实环境供应或设备协议已经整体完成。
