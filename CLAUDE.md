...
- 当前仓库已由 F-03 收口为最终九模块 reactor。独立目标数据库迁移已推进到 V40：V35 的 99 张领域表经 V36～V38 收口永久设备资产和机器验收，V39 再把机构独占 AppID/OpenID 改为共享小程序渠道、微信主体和独立机构账号，V40 补齐设备边缘事件从旧部署目标到永久资产目标的前向切换，共 95 张领域表、76 条有效权限定义。服务器实际版本仍必须在部署前现场核对并用 H-02 续跑到 V40，不能仅凭旧记录假定已经升级。
...
4. 判断“系统现在如何运行”时，以当前代码、测试、根 `pom.xml`、旧运行 Flyway V1～V14、独立目标迁移 `db/p0-migration/V1～V40`、当前 OneNet v2 物模型和设备程序为准。
...
| 数据库 | 旧栈恢复单元仍是 V1～V14/13 张主要表；目标 V1～V40 为 95 张领域表，迁移只由独立 Maven 作业执行，新运行制品不含 Flyway 运行库或迁移脚本，并以固定 V1 marker + V40 guard 检查；服务器实际版本部署前必须现场核对并补齐到 V40 | 新旧应用/数据库成对隔离；V36 是无生产设备数据前提下的破坏式设备模型切换，V37/V38 是其前向修订，V39 是共享小程序身份模型的前向切换，V40 删除边缘事件中的旧部署目标语义 |
...
3. 新数据库 V1～V40 的 DDL/Flyway、种子数据、数据库账号和旧新应用/数据库成对切换；
