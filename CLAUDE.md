...
- D-047/I-057 的权威裁决是 [`permanent-device-ownership-v36.md`](docs/architecture/permanent-device-ownership-v36.md)：一台资产只允许永久分配一次租户和机构，平台分配前自动真机验收，机构只安装联网；部署实例、部署码、回收/调拨、人工现场验收和人工经营开关全部退出，不保留旧 API、旧协议或边缘数据库兼容层。
- 当前仓库已由 F-03 收口为最终九模块 reactor。独立目标数据库迁移已推进到 V36：V35 的 99 张领域表经 V36 删除旧设备模型并新增验收/出厂袋事实后，最终为 93 张领域表、76 条有效权限定义；已在一次性 MySQL 8.4.10 上通过 V1→V36、最小权限、纪元门禁和只读启动验证。服务器实际版本仍必须在部署前现场核对并用 H-02 续跑到 V36，不能仅凭旧记录假定已经升级。
...
4. 判断“系统现在如何运行”时，以当前代码、测试、根 `pom.xml`、旧运行 Flyway V1～V14、独立目标迁移 `db/p0-migration/V1～V36`、当前 OneNet v2 物模型和设备程序为准。
...
| 数据库 | 旧栈恢复单元仍是 V1～V14/13 张主要表；目标 V1～V36 为 93 张领域表，迁移只由独立 Maven 作业执行，新运行制品不含 Flyway 运行库或迁移脚本，并以固定 V1 marker + V36 guard 检查；服务器实际版本部署前必须现场核对并补齐到 V36 | 新旧应用/数据库成对隔离；V36 是无生产设备数据前提下的破坏式设备模型切换，不迁移旧部署数据 |
...
- 一台物理设备就是一条永久资产记录：平台自动真机验收通过后才能永久分配一次租户，租户再永久分配一次机构；两个归属都不可撤销、调拨或重新分配。机构只安装联网，不确认验收、不启用经营开关。
...
3. 新数据库 V1～V36 的 DDL/Flyway、种子数据、数据库账号和旧新应用/数据库成对切换；
